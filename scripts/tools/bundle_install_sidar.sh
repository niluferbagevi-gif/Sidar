...
REPO_HASHES="${MODULE_DIR}/MODULE_HASHES.txt"
...
{
    printf '# Sidar install module SHA256 hashes\n'
    printf '# Generated by scripts/tools/bundle_install_sidar.sh at %s\n' "$BUILD_DATE"
    (
        cd "$MODULE_DIR"
        find . -type f \( -name '*.sh' -o -name '*.ps1' \) ! -name '*.sha256' | sort \
            | while read -r module_path; do
                module_path="${module_path#./}"
                module_sha="$(sha256sum "$module_path" | awk '{print $1}')"
                printf '%s  %s\n' "$module_sha" "$module_path"
                printf '%s\n' "$module_sha" > "${module_path}.sha256"
            done
    )
} > "$OUTPUT_HASHES"

cp "$OUTPUT_HASHES" "$REPO_HASHES"
echo "Bundle oluşturuldu: $OUTPUT_SCRIPT"
echo "Modül hash manifesti oluşturuldu: $OUTPUT_HASHES"
echo "Repo modül hash manifesti güncellendi: $REPO_HASHES"
