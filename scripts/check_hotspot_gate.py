...
def _normalize_filename(package_name: str, filename: str) -> str:
    if "/" in filename:
        return filename
    if package_name:
        package_path = package_name.replace(".", "/")
        return f"{package_path}/{filename}"
    return filename
...
def parse_coverage_xml(xml_path: Path) -> dict[str, FileCoverage]:
    ...
    for package in xml_root.findall(".//package"):
        ...
        for cls in package.findall(".//class"):
            ...
            normalized = _normalize_filename(package_name=package_name, filename=filename)
            files[normalized] = FileCoverage(covered=covered, missed=missed)
...
def check_hotspots(
    ...
    def _resolve_hotspot_path(path: str) -> FileCoverage | None:
        direct = current.get(path)
        if direct is not None:
            return direct

        # Fallback 1: allow keys with extra source-root prefixes.
        suffix_matches = [rec for key, rec in current.items() if key.endswith(f"/{path}")]
        if len(suffix_matches) == 1:
            return suffix_matches[0]

        # Fallback 2: drop first path segment (agent/, core/, managers/...) and match suffix.
        if "/" in path:
            tail = path.split("/", 1)[1]
            tail_matches = [rec for key, rec in current.items() if key.endswith(tail)]
    ...
    for hotspot in config.get("hotspots", []):
        ...
        record = _resolve_hotspot_path(path)
        if record is None:
