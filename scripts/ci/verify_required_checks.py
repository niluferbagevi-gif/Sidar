...
def _repo_from_git_remote() -> str:
    ...
    try:
        remote = subprocess.check_output(  # nosec B603 B607  # noqa: S603 - fixed git command without user shell.
...
def _fetch_required_contexts(
    *, api_url: str, repo: str, branch: str, token: str | None, timeout: float
    ...
    try:
        with urlopen(request, timeout=timeout) as response:  # nosec B310  # noqa: S310 - URL is GitHub API or explicit operator override.
