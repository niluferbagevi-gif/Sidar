...
SIDAR_INSTALL_UTIL_OLLAMA_RUNTIME_SH_LOADED=1

# Ollama (default AI provider) runtime install + healthcheck helper.
# Split out of install_sidar.sh:ensure_prerequisites so that the install flow
# stays consistent with the modular phase/util architecture under
# scripts/install_modules/. The sibling util ollama_models.sh handles the
# post-install model provisioning step; this util is intentionally limited to
# binary acquisition, supply-chain checksum gating, sudo orchestration and the
# first-pass API healthcheck.
