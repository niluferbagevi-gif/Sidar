...
seed_rag_in_docker_after_startup() {
    ...
    if command -v docker &>/dev/null && docker compose version &>/dev/null; then
        ...
        warn "Docker compose bulunamadı; RAG warmup seed atlandı. Manuel: docker compose run --rm --no-deps --entrypoint \"\" sidar-web uv run python -m scripts.seed_rag"
        return 0
    fi

    info "Tam Docker modu: ilk açılış gecikmesini azaltmak için RAG/GraphRAG seed adımı çalıştırılıyor..."
    if (cd "$SCRIPT_DIR" && "${compose_cmd[@]}" run --rm --no-deps --entrypoint "" sidar-web uv run python -m scripts.seed_rag); then
        ...
        warn "Docker RAG/GraphRAG seed adımı başarısız. Manuel: ${compose_cmd[*]} run --rm --no-deps --entrypoint \"\" sidar-web uv run python -m scripts.seed_rag"
    fi
}
