    ...
    if "${PY_CMD[@]}" -c "import playwright" >/dev/null 2>&1; then
        ...
        _ensure_playwright_override_dependencies() {
            ...
            if _try_playwright_install_deps; then
                # install-deps bazen Chromium sistem bağımlılıklarını kuramadığında
                # bile exit 0 döner (BEWARE/"Cannot install dependencies" ile);
                # bu durumda ham çıktıyı kullanıcıya göstermeden önce
                # playwright_linux_dependencies_ready() gerçek durumu doğrular.
