...
def test_rag_add_file_validation_errors(client: TestClient) -> None:
    empty = client.post("/rag/add-file", json={})
    assert empty.status_code == 400
    assert "Dosya yolu boş" in empty.json()["error"]

    traversal = client.post("/rag/add-file", json={"path": "../../etc/passwd"})
...
def test_rag_add_file_invalid_json_results_500(client: TestClient) -> None:
    response = client.post("/rag/add-file", data="not-json", headers={"Content-Type": "application/json"})
    assert response.status_code == 500


def test_rag_add_url_empty_and_success(client: TestClient) -> None:
    empty = client.post("/rag/add-url", json={})
    assert empty.status_code == 400
    assert "URL boş" in empty.json()["error"]

    ok = client.post("/rag/add-url", json={"url": "https://example.com/doc", "title": "Doc"})
    assert ok.status_code == 200
    assert ok.json()["success"] is True


def test_rag_delete_and_list_docs(client: TestClient) -> None:
    listed = client.get("/rag/docs")
    assert listed.status_code == 200
    assert listed.json()["count"] == 1

    deleted = client.delete("/rag/docs/doc-1")
