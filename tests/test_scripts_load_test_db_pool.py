...
class TestRunOnce:
    @pytest.mark.asyncio
    async def test_run_once_returns_positive_float(self):
    ...
    @pytest.mark.asyncio
    async def test_run_once_executes_select_1(self):
        db, mock_conn = _make_mock_db()
        _stub_script_deps(db)
        mod = _import_script()
    ...
    @pytest.mark.asyncio
    async def test_run_once_requires_pg_pool(self):
        db, _ = _make_mock_db()
        db._pg_pool = None
        _stub_script_deps(db)
        mod = _import_script()
...
class TestRunLoadTest:
    @pytest.mark.asyncio
    async def test_basic_run_prints_ok(self, capsys):
    ...
    @pytest.mark.asyncio
    async def test_non_postgresql_raises_runtime_error(self):
    ...
    @pytest.mark.asyncio
    async def test_db_close_called_after_test(self):
        db, _ = _make_mock_db()
        _stub_script_deps(db)
        mod = _import_script()
        await mod.run_load_test(
    ...
    @pytest.mark.asyncio
    async def test_db_close_called_even_on_error(self):
    ...
    @pytest.mark.asyncio
    async def test_pool_size_env_set(self):
        import os
        db, _ = _make_mock_db()
        _stub_script_deps(db)
        mod = _import_script()
    ...
    @pytest.mark.asyncio
    async def test_concurrency_capped_at_100(self, capsys):
    ...
    @pytest.mark.asyncio
    async def test_output_contains_metrics(self, capsys):
        db, _ = _make_mock_db()
        _stub_script_deps(db)
        mod = _import_script()
        await mod.run_load_test(
    ...
    @pytest.mark.asyncio
    async def test_single_request(self, capsys):
        db, _ = _make_mock_db()
        _stub_script_deps(db)
        mod = _import_script()
        await mod.run_load_test(
