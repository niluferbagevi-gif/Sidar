...
def test_list_repos_create_pr_and_read_file_return_errors_on_exceptions() -> None:
    class _GHStub:
    ...
    class _RepoStub:
        ...
        def create_pull(self, **_kwargs):
            raise RuntimeError("cannot create pr")

        def get_pulls(self, **_kwargs):
            raise RuntimeError("cannot list prs")

    manager = _GHManagerNoInit(token="t", repo_name="", require_token=False)
    manager._gh = _GHStub()
    manager._repo = _RepoStub()

    ok_repos, repos = manager.list_repos(owner="octo", limit=5)
    ok_read, read_msg = manager.read_remote_file("README.md")
    ok_pr, pr_msg = manager.create_pull_request("title", "body", "feature/x")
    ok_list_pr, list_pr_msg = manager.list_pull_requests(state="open", limit=5)

    assert ok_repos is False
    assert repos == []
    assert ok_read is False
    assert "Uzak dosya okunamadı" in read_msg
    assert ok_pr is False
    assert "Pull Request oluşturma hatası" in pr_msg
    assert ok_list_pr is False
    assert "PR listesi alınamadı" in list_pr_msg
