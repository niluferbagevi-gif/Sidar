...
def test_notify_returns_immediately_when_no_broadcast_hook_is_registered():
    import core.hitl as hitl_mod

    async def _inner():
        now = time.time()
        req = HITLRequest(
            request_id="notify-no-hook",
            action="overwrite",
            description="Hook yok",
            payload={},
            requested_by="tester",
            created_at=now,
            expires_at=now + 30,
...
def test_pending_skips_requests_that_are_not_pending():
    async def _inner():
        store = _HITLStore()
        now = time.time()
        approved = HITLRequest(
            request_id="approved-id",
            action="overwrite",
            description="Önceden onaylandı",
            payload={},
            requested_by="tester",
            created_at=now,
            expires_at=now + 60,
        ...
        assert [req.request_id for req in result] == ["pending-id"]
        assert approved.decision == HITLDecision.APPROVED
...
def test_request_approval_timeout_path_leaves_non_pending_request_unchanged(monkeypatch, caplog):
    async def _inner():
        import core.hitl as hitl_mod
        ...
        approved = await gate.request_approval(
            action="dangerous_write",
            description="Süre dolduktan sonra karar değişmemeli",
            payload={"path": "/tmp/demo.py"},
            requested_by="CodeManager",
        )

        assert approved is False
        assert seen == [store._requests[-1].request_id]
        assert store._requests[-1].decision == HITLDecision.APPROVED
        assert store._requests[-1].decided_by == "operator"
        assert store._requests[-1].decided_at is None

    with caplog.at_level("WARNING"):
        _run(_inner())

    assert "HITL zaman aşımı" in caplog.text
