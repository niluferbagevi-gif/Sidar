...
async def test_basic_auth_middleware_auth_paths(monkeypatch):
    ...
    assert github_webhook_res.status_code == 200

    autonomy_webhook_res = await web_server.basic_auth_middleware(
        _make_request("/api/autonomy/webhook/github", method="POST"), _ok_next
    )
    assert autonomy_webhook_res.status_code == 200

    webhook_get_denied = await web_server.basic_auth_middleware(
        _make_request("/api/webhook", method="GET"), _ok_next
    )
    assert webhook_get_denied.status_code == 401
...
def test_plugin_source_execution_production_cannot_be_overridden(monkeypatch):
    monkeypatch.setenv("SIDAR_ENV", "production")
    monkeypatch.setenv("SIDAR_ENABLE_IN_PROCESS_PLUGINS", "1")

    with pytest.raises(HTTPException) as exc:
        web_server._run_plugin_source_in_sandbox("VALUE = 1", "prod_override_rejected")

    assert exc.value.status_code == 403
    assert "production kısıtını kaldıramaz" in exc.value.detail
...
async def test_github_webhook_unsigned_local_secret_empty_success(monkeypatch):
    ...
    monkeypatch.delenv("GITHUB_WEBHOOK_REQUIRE_SIGNATURE", raising=False)
    ...
    def _verify_should_not_run(*args, **kwargs):
        ...
        raise AssertionError("empty local GitHub webhook secret should skip HMAC verification")

    monkeypatch.setattr(web_server, "_verify_hmac_signature", _verify_should_not_run)
...
def test_github_webhook_signature_validation_contract():
    ...
    assert result == "disabled_by_flag"
    assert not any(call[0] == "verify" for call in calls)
...
async def test_github_webhook_handles_sync_await_helper_result(monkeypatch):
    ...
    monkeypatch.setattr(web_server, "_dispatch_autonomy_trigger", _dispatch_sync_helper)
...
async def test_websocket_chat_auth_message_rejects_invalid_token(monkeypatch):
    class _Ws:
        def __init__(self):
            ...
            self.client = SimpleNamespace(host="127.0.0.1")
            self.closed = None
            self.messages = iter(['{"action":"auth","token":"bad-token"}'])
...
async def test_websocket_chat_streams_tool_thought_and_done_packets(monkeypatch):
    ...
    class _Agent:
        ...
        async def respond(self, _prompt):
            yield "\x00TOOL:web_search\x00"
            yield "\x00THOUGHT:analiz\x00"
            yield "tamam"
    ...
    assert {"chunk": "tamam"} in ws.payloads
    assert {"done": True} in ws.payloads
...
async def test_websocket_chat_status_pump_stops_cleanly_when_flag_set(monkeypatch):
    ...
    async def _resolve_user(*_):
        return SimpleNamespace(id="", username="ada", role="developer")

    async def _not_limited(*_args, **_kwargs):
        return False

    original_create_task = web_server.asyncio.create_task
    ...
    monkeypatch.setattr(web_server.asyncio, "create_task", _patched_create_task)
...
async def test_websocket_chat_generate_response_finally_skips_unset_sub_and_ctx(monkeypatch):
    ...
    ws = _Ws()
    await web_server.websocket_chat(ws)
    assert any("[Sistem Hatası]" in item.get("chunk", "") for item in ws.payloads)
...
async def test_websocket_chat_join_room_and_sidar_command_stream(monkeypatch):
        ...
        def __init__(self):
            ...
            self.messages = iter(
                [
                    ...
                    '{"action":"message","message":"@sidar README güncelle"}',
                ]
            )
    ...
    types = [payload.get("type") for payload in ws.payloads if isinstance(payload, dict)]
    assert "room_message" in types
    assert "assistant_stream_start" in types
...
async def test_websocket_chat_room_status_pump_stops_cleanly_when_flag_set(monkeypatch):
    ...
    class _Ws:
        def __init__(self):
            ...
            self.messages = iter(
                [
                    '{"action":"join_room","room_id":"team:eta","display_name":"Ada"}',
                    '{"action":"message","message":"@sidar planı güncelle"}',
                ]
            )
    ...
    class _Ws:
        ...
        async def receive_text(self):
            try:
                ...
                raise _AnyIoClosed("closed") from None
...
async def test_websocket_chat_message_status_stream_emits_and_cleans_context(monkeypatch):
    ...
    class _EventBus:
        def subscribe(self):
            q: asyncio.Queue = asyncio.Queue()
            q.put_nowait(SimpleNamespace(source="reviewer", message="analiz başladı"))
            return "sub-status", q
    ...
    class _Agent:
        ...
        async def respond(self, *_):
            await asyncio.sleep(0.55)  # status pump timeout=0.5 için en az bir tur daha
            yield "tamam"
    ...
    class _Ws:
        def __init__(self):
            ...
            self._messages = iter(
                [
                    json.dumps({"action": "auth", "token": "ok"}),
                    json.dumps({"action": "message", "message": "durum nedir?"}),
                ]
            )
        ...
        async def send_json(self, payload):
            if "status" in payload:
                calls["statuses"].append(payload["status"])
            self.sent.append(payload)
    ...
    assert {"auth_ok": True} in ws.sent
    assert any("reviewer: analiz başladı" == status for status in calls["statuses"])
...
async def test_websocket_chat_room_status_stream_timeout_and_task_cleanup(monkeypatch):
    ...
    class _EventBus:
        def subscribe(self):
            q: asyncio.Queue = asyncio.Queue()
            q.put_nowait(SimpleNamespace(source="qa", message="oda analizi başlatıldı"))
            return "sub-room-status", q
    ...
    class _Agent:
        ...
        async def _try_multi_agent(self, *_):
            await asyncio.sleep(0.55)  # queue event sonrası timeout branch'ini zorlar
            return "oda tamam"
    ...
    async def _capture_broadcast(room, payload):
        calls["broadcast"].append((room.room_id, payload))
        await asyncio.sleep(0)
