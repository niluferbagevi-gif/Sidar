...
@pytest.mark.asyncio
async def test_websocket_chat_times_out_waiting_for_first_auth_message() -> None:
    class _SlowAuthWs(_Ws):
        async def receive_text(self) -> str:
            await ws_chat.asyncio.sleep(0.05)
            return '{"action":"auth","token":"late"}'
