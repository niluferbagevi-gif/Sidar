...
def test_repository_toolchain_surfaces_match_canonical_file() -> None:
    ...
    assert pins["PLAYWRIGHT_NODE_MAX"] == "1.63.0"
...
def test_required_node_uses_nvmrc_and_node_22_is_non_blocking_compatibility() -> None:
    ...
    compatibility = workflow[
        workflow.index("  frontend-node-compatibility:") : workflow.index(
            "  production-compose-validation:"
        )
    ]
    assert "continue-on-error: true" in compatibility
    assert "node: [22]" in compatibility
    assert "npm run test:critical" in compatibility
    assert "npm run build" in compatibility
