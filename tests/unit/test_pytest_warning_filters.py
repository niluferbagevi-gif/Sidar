...
def test_pytest_filters_include_sentence_transformers_invalid_escape_rules() -> None:
    ...
    assert "ignore:invalid escape sequence '\\d':DeprecationWarning:sentence_transformers.model_card" in filters
    assert (
        "ignore:invalid escape sequence '\\_':DeprecationWarning:sentence_transformers.losses.DenoisingAutoEncoderLoss"
        in filters
    )
