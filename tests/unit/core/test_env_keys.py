...
def test_initialize_env_file_copies_example_and_generates_local_secrets(tmp_path):
    ...
    assert len(base64.urlsafe_b64decode(values["MEMORY_ENCRYPTION_KEY"])) == 32
...
def test_initialize_env_file_preserves_existing_strong_values_without_force(tmp_path):
    ...
    env.write_text(
        "API_KEY=existing-api-key-value-with-more-than-24-chars\n"
        "JWT_SECRET_KEY=\n",
        encoding="utf-8",
    )
    ...
    assert values["API_KEY"] == "existing-api-key-value-with-more-than-24-chars"
...
def test_initialize_env_file_force_rotates_existing_values(tmp_path):
    ...
    env = tmp_path / ".env"
    example.write_text("API_KEY=\n", encoding="utf-8")
    env.write_text("API_KEY=existing-api-key-value-with-more-than-24-chars\n", encoding="utf-8")
