...
def test_gpu_memory_config_confirms_standard_local_model(monkeypatch):
    ...
    monkeypatch.setattr(doctor, "_docker_image_exists_locally", lambda _img: True)
...
def test_gpu_memory_config_warns_for_default_slim_test_image(monkeypatch):
    ...
    monkeypatch.setattr(doctor, "_docker_image_exists_locally", lambda _img: False)
    ...
    assert any("DOCKER_TEST_IMAGE=sidar:latest" in cmd for cmd in check.details["recommended_commands"])


def test_gpu_memory_config_warns_when_sidar_latest_image_missing(monkeypatch):
    from config import Config
    ...
    assert check.status == "warn"
    assert "not present locally" in check.message
    assert check.details["sidar_latest_image_present"] is False
    assert "docker build -t sidar:latest ." in check.details["recommended_commands"]
