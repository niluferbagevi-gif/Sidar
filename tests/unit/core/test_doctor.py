...
def test_database_env_fails_when_url_missing_but_password_present(monkeypatch):
    """A reload that drops DATABASE_URL while POSTGRES_PASSWORD survives is a regression."""
    monkeypatch.delenv("DATABASE_URL", raising=False)
    monkeypatch.delenv("SIDAR_CONTAINER_DATABASE_URL", raising=False)
    ...
    assert "URL was likely lost during env reload" in check.message
    assert check.details["database_url_set"] is False
