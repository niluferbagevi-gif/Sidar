...
async def test_retry_with_backoff_raises_llm_api_error() -> None:
    async def op():
        raise ValueError("fatal")

    with pytest.raises(llm_client.LLMAPIError) as exc:
        await llm_client._retry_with_backoff("openai", op, config=_make_config(), retry_hint="retry")
...
async def test_semantic_cache_cosine_similarity() -> None:
...
async def test_semantic_cache_get_hit(monkeypatch: pytest.MonkeyPatch) -> None:
    manager = llm_client._SemanticCacheManager(_make_config())
    fake = _FakeRedis()
    fake.index = ["k1"]
...
async def test_semantic_cache_get_returns_none_without_prompt() -> None:
    manager = llm_client._SemanticCacheManager(_make_config())
    assert await manager.get("") is None


async def test_semantic_cache_get_records_miss(monkeypatch: pytest.MonkeyPatch) -> None:
...
async def test_semantic_cache_set_records_item(monkeypatch: pytest.MonkeyPatch) -> None:
    manager = llm_client._SemanticCacheManager(_make_config())
    fake = _FakeRedis()
...
async def test_semantic_cache_set_skips_without_response() -> None:
    manager = llm_client._SemanticCacheManager(_make_config())
...
async def test_ollama_client_chat_non_stream_and_stream(monkeypatch: pytest.MonkeyPatch) -> None:
...
async def test_ollama_stream_response_parses_and_handles_error(monkeypatch: pytest.MonkeyPatch) -> None:
    cfg = _make_config()
    client = llm_client.OllamaClient(cfg)
...
async def test_ollama_list_models_and_availability(monkeypatch: pytest.MonkeyPatch) -> None:
...
async def test_litellm_candidate_and_chat(monkeypatch: pytest.MonkeyPatch) -> None:
...
async def test_litellm_stream_and_fail(monkeypatch: pytest.MonkeyPatch) -> None:
...
async def test_gemini_client_missing_and_success(monkeypatch: pytest.MonkeyPatch) -> None:
...
async def test_anthropic_helpers_and_chat(monkeypatch: pytest.MonkeyPatch) -> None:
...
async def test_llmclient_wrapper_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    chunks = (await _collect(await client.chat([{"role": "user", "content": "hello"}], stream=True, json_mode=False)))
    assert chunks == ["x"]
...
async def test_semantic_cache_get_set_error_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    manager = llm_client._SemanticCacheManager(_make_config())
    fake = _FakeRedis()
...
async def test_llmclient_routing_and_compat_helpers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(llm_client, "_dlp_mask_messages", lambda m: m)
    monkeypatch.setattr(c._semantic_cache, "get", AsyncMock(return_value=None))
    monkeypatch.setattr(c._semantic_cache, "set", AsyncMock(return_value=None))
...
async def test_semantic_cache_get_handles_invalid_records(monkeypatch: pytest.MonkeyPatch) -> None:
    manager = llm_client._SemanticCacheManager(_make_config(SEMANTIC_CACHE_THRESHOLD=0.99))
    fake = _FakeRedis()
    fake.index = ["k1", "k2", "k3"]
    fake.hashes["k1"] = {}
...
async def test_llmclient_provider_helpers_and_cache_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(c._client, "chat", AsyncMock(return_value="ok"))
    monkeypatch.setattr(c._semantic_cache, "get", AsyncMock(return_value=None))
    monkeypatch.setattr(c._semantic_cache, "set", AsyncMock(return_value=None))
...
async def test_semantic_cache_additional_branches(monkeypatch: pytest.MonkeyPatch) -> None:
    manager = llm_client._SemanticCacheManager(_make_config())
    fake = _FakeRedis()
    fake.index = ["k1", "k2"]
