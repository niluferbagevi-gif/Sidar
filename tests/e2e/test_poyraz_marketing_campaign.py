...
async def test_poyraz_research_copy_and_publish_campaign_flow():
    ...
    research = await asyncio.wait_for(
        agent.run_task("web_search|instagram kampanya trendleri"), timeout=10
    )
    campaign_copy = await asyncio.wait_for(
        agent.run_task("generate_campaign_copy|Yeni ürün lansmanı için metin hazırla"), timeout=10
    )
