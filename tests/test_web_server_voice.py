...
def test_websocket_voice_anyio_closed_without_active_task_exits_gracefully(monkeypatch):
    mod = _load_web_server()
    info_logs = []
    ...
    voice_mod = types.ModuleType("core.voice")
    multimodal_mod.MultimodalPipeline = lambda *_a, **_k: object()
    voice_mod.VoicePipeline = lambda *_a, **_k: types.SimpleNamespace(
        create_duplex_state=lambda: types.SimpleNamespace(assistant_turn_id=0, output_text_buffer="", last_interrupt_reason=""),
        vad_enabled=False,
    ...
    assert ws.accepted == "valid-token"
    assert any("ClosedResourceError" in message for message in info_logs)
