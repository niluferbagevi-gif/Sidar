...
### Düzeltmeler
- **`configure_gpu_env_defaults()`, her GPU'lu kurulumda `info` seviyesinde koşulsuz bir iç implementasyon mesajı (`"DEBUG: GPU branch entered for .env configuration (GPU_AVAILABLE=true)."`) yazdırıyordu — bir arkadaş kod incelemesinde tespit edildi:** Normal (debug olmayan) bir kullanıcı kurulum logunda bu satır her zaman görünüyordu; işlevsel bir hata değil ama production kalitesinde installer çıktısında olmaması gereken bir iç durum notu. `install_sidar.sh` zaten tam bu amaç için `debug()` helper'ını taşıyor (`SIDAR_DEBUG=1`/`SIDAR_VERBOSE=1` altında gösterir, aksi halde sessiz — `03_system.sh`/`12_alembic.sh`'de zaten bu şekilde kullanılıyor); yeni bir `SIDAR_INSTALL_DEBUG` bayrağı icat etmek yerine satır `debug "GPU branch entered for .env configuration (GPU_AVAILABLE=true)."`'a çevrildi, mevcut tek debug-gating konvansiyonuyla tutarlı kalındı. `tests/shell/install_phase08_env.bats`'e regresyon testi eklendi: `SIDAR_DEBUG` set değilken mesajın çıktıda hiç görünmediğini, `SIDAR_DEBUG=1` iken göründüğünü doğruluyor. `install_sidar.sh` gömülü modül hash manifesti yeniden senkronize edildi.
- **İnteraktif API anahtarı toplama sihirbazı (`08_env.sh`), `SIDAR_USER_SECRET_ENV_KEYS`'e daha önce eklenen 3 anahtarı hiç sormuyordu — bir arkadaş kod incelemesinde tekrar gündeme geldi:** Bir önceki turda (bkz. aşağıdaki `test_env_example_secret_keys_are_all_in_the_install_masking_allowlist` maddesi) `GOOGLE_API_KEY`/`JIRA_API_TOKEN`/`META_GRAPH_API_TOKEN` log maskeleme + raporlama allowlist'i olan `SIDAR_USER_SECRET_ENV_KEYS`'e eklenmişti, ama o düzeltme bilinçli olarak `collect_api_keys_interactive()`'in ayrı, elle tutulan `API_GROUPS` dizisine dokunmamıştı (changelog'da açıkça "interaktif kurulum sihirbazının hangi anahtarları soracağını ETKİLEMEDİĞİNİ doğruladım" diye not edilmişti). Bu incelemede kalan boşluk netleşti: `API_GROUPS` (zenity/whiptail/read'in üçü de aynı diziyi dolaşıyor) 21 kullanıcı secret'ından yalnızca 18'ini içeriyordu; kullanıcı kuruluma bu üç anahtarı asla gireme fırsatı bulamıyor, secret overlay'de sessizce boş kalıyordu — canlı bir kurulum log'unda 18/21 dolu secret overlay olarak doğrulandı. Kök neden: iki allowlist arasında (`SIDAR_USER_SECRET_ENV_KEYS` — masking/raporlama, `API_GROUPS` — interaktif toplama) hiçbir otomatik tutarlılık kontrolü yoktu. `API_GROUPS`'ın elle yazımı, yeni `_api_groups_definition()` fonksiyonuna taşındı (tek yer, hem `collect_api_keys_interactive()` hem testler tarafından çağrılabilir); `GOOGLE_API_KEY` "AI Sağlayıcıları" grubuna, `JIRA_API_TOKEN` "Jira" grubuna, `META_GRAPH_API_TOKEN` yeni bir "Meta" grubuna eklendi; `_key_label()`'a üçü için kullanıcı dostu etiketler eklendi. Yeni `tests/shell/install_phase08_env.bats::"phase 08 API_GROUPS covers every SIDAR_USER_SECRET_ENV_KEYS key exactly once"` — `install_sidar.sh`'daki gerçek `SIDAR_USER_SECRET_ENV_KEYS` dizisini (sourcing yerine yalnızca dizi bildirimini `sed`/`eval` ile çıkararak, tüm installer'ı çalıştırmadan) `_api_groups_definition()`'ın düzleştirilmiş çıktısıyla karşılaştırıp her anahtarın tam bir kez eşleştiğini doğruluyor — eski (18 anahtarlı) tanıma karşı çalıştırıldığında gerçekten aynı 3 eksik anahtarı yakaladığı doğrulandı, bundan böyle 22. bir anahtar eklenip UI grubunun unutulması CI'da kırmızı olacak. `install_sidar.sh`'ın gömülü modül hash manifesti (`scripts/sync_install_module_hashes.sh`) yeniden senkronize edildi; `bash -n` ile hem `install_sidar.sh` hem `08_env.sh` syntax-temiz.
