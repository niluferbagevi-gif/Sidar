...
### Düzeltmeler (Fixed)
- **`OLLAMA_INSTALL_SHA256`/`UV_INSTALL_SHA256` eksikken interaktif review-and-pin akışı gerçek bir terminalde bile hiç tetiklenmiyordu, operatör her seferinde manuel `export ... && ./install_sidar.sh` tekrar çalıştırmaya zorlanıyordu:** `install_sidar.sh` erkenden loglama için kendi stdout/stderr'ini bir `tee` pipe'ına yönlendiriyor (`exec > >(mask_install_log_stream | tee ...) 2>&1`); bu satırdan sonra canlı `[[ -t 1 ]]` kontrolü, gerçek interaktif bir terminalde çalışılsa dahi kalıcı olarak false dönüyor (fd 1 artık bir pipe). `scripts/install_modules/utils/remote_script.sh:review_and_pin_remote_script_checksum()` bu canlı `-t 1` değerine bakarak interaktifliği kontrol ettiği için, checksum eksikliğinde betiği inceleyip onaylayarak SHA-256'yı o oturum için otomatik üretip kuruluma devam etme akışı asla devreye girmiyor, doğrudan "NEXT STEP → ..._SHA256=<hash> ./install_sidar.sh" manuel-düzeltme hatasına düşülüyordu. `install_sidar.sh` artık loglama redirect'inden HEMEN ÖNCE gerçek tty durumunu `SIDAR_INSTALL_ORIGINAL_STDOUT_IS_TTY`'e sabitleyip export ediyor; `review_and_pin_remote_script_checksum()` artık canlı `-t 1` yerine bu değeri (ve redirect'ten etkilenmeyen `-t 0`'ı) kullanıyor, pager çağrısı da `< /dev/tty > /dev/tty` ile açıkça terminale bağlanıyor. Sonuç: gerçek bir terminalde checksum eksik hatası artık betiği incele → onayla → SHA-256 otomatik pinlenip kuruluma aynı oturumda devam et akışını tetikliyor; CI/non-interaktif/piped ortamlarda davranış (fail-fast + manuel talimat) değişmedi. `tests/unit/scripts/test_run_tests_quality_gate.py::test_install_sidar_review_and_pin_survives_log_tee_stdout_redirect` regresyon testi eklendi.
