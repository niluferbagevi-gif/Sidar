...
### Düzeltmeler (Fixed)
- **`core.doctor`'daki `rag_index_ready`/`graphrag_entity_memory_ready`/`check_rag_readiness` kontrolleri, `DATABASE_URL` yalnızca `POSTGRES_*` parçalarından türetildiğinde (installer'ın kasıtlı varsayılan davranışı) pgvector'ü hep "bloke" olarak raporluyordu:** `check_database_env()` ve `check_database_connectivity()` zaten `_resolved_database_urls()` üzerinden DATABASE_URL'i (açık tanım yoksa) `POSTGRES_USER/PASSWORD/DB`'den türetiyordu, ama `_rag_readiness_state()` (satır ~1022) ve `check_rag_readiness()` (satır ~1257) doğrudan `os.getenv("DATABASE_URL", "")` okuyordu. Sonuç: bu iki fonksiyon açık `DATABASE_URL` yazılmamış (yalnızca türetilmiş) her sağlıklı kurulumda `database_env_status="fail"` görüp `rag_index_ready`/`graphrag_entity_memory_ready`'yi yanlışlıkla "blocked" işaretliyordu — `database_env` ve `pgvector_ready` kontrolleri aynı anda "pass" dönse bile. Bu, tam olarak satır 19-23'te düzeltilen `websocket_routes`/`database_connectivity`/`gpu_memory_config`/`redis` "ham os.getenv vs. gerçek çözücü" deseninin RAG tarafında atlanmış son örneğiydi. Her iki fonksiyon da artık `_resolved_database_urls()` kullanıyor. `tests/unit/core/test_doctor.py`'a `POSTGRES_*`'ten türetilmiş (açık `DATABASE_URL` olmayan) pgvector ortamının artık yanlışlıkla bloke edilmediğini doğrulayan iki regresyon testi eklendi.
