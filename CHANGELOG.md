...
- **`core.doctor`'daki `rag_index_ready`/`graphrag_entity_memory_ready`/`check_rag_readiness` kontrolleri, `database_env` pas geçse bile pgvector'ü yanlışlıkla "blocked" işaretliyordu:** `check_database_env()` ve `check_database_connectivity()` `_resolved_database_urls()` üzerinden çalışıyor — bu yardımcı fonksiyon, DATABASE_URL dotenv'de açıkça yazılı olmadığında (Sidar'ın kasıtlı tercih ettiği güvenli desen) onu POSTGRES_USER/PASSWORD/DB parçalarından türetiyor. `_rag_readiness_state()` (pgvector dalı) ve `check_rag_readiness()` ise `_resolved_database_urls()` yerine ham `os.getenv("DATABASE_URL", "")` okuyordu; DATABASE_URL dosyalarda açık yazılı olmayan (yalnızca POSTGRES_* türetilen) kurulumlarda bu iki fonksiyon boş string görüyor ve `database_env_status` gerçekte "pass" olmasına rağmen sürekli "fail"/`blocked_by: database_env` üretiyordu — pgvector fiilen sağlıklı çalışırken (`pgvector_ready` `select_1`/`pgvector_extension_installed` ile "pass" diyor) Doctor kullanıcıyı gereksiz yere `sync_database_passwords --remove-explicit-urls` çalıştırmaya yönlendiriyordu (zaten hiçbir şeyi değiştirmeyen bir komut, çünkü bozuk olan DATABASE_URL değil onu okuyan kontroldü). `core/doctor/__init__.py:1022` ve `:1257`'deki ham `os.getenv("DATABASE_URL", "")` çağrıları `_resolved_database_urls()[0]` ile değiştirildi (CHANGELOG'daki websocket_routes/database_connectivity/gpu_memory_config/redis düzeltmeleriyle aynı "ham os.getenv vs. gerçek çözücü" deseninin tedavi edilmemiş son kalıntısıydı). `tests/unit/core/test_doctor.py`'a DATABASE_URL dotenv'de yokken yalnızca POSTGRES_*'ten türetildiği senaryoda `_rag_readiness_state`/`check_rag_readiness`'in artık yanlışlıkla "blocked" dönmediğini doğrulayan iki regresyon testi eklendi.
