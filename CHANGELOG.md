...
### Düzeltmeler
- **Üretim kodundaki 39 modül/paket dosyası docstring'siz kalmıştı:** `agent/`, `core/`, `managers/`, `plugins/`, `scripts/` ve `web/routes/` altındaki modüllere amaçlarını anlatan docstring eklendi; üretim kodunda D100/D104 bulgusu kalmadı. Ratchet D100 227→191, D104 12→9 olarak sıkılaştırıldı.
- **Bandit suppression hedefi 2027-08-01'de CI'ı kod düzeltmesiyle giderilemez biçimde kıracaktı:** 2027-07-31/0 hedefi, kalan 8 suppression'ın Bandit kaynak kodu düzeyinde kaçınılmaz olduğu kanıtlandığı için gerekçesiyle emekliye ayrıldı. Yerine tarihli `structural_floor` (8, `review_by=2027-07-31`) geldi; tarih geçince kapı yine fail-closed kırılır.
- **pip-audit istisnalarının `next_review` tarihleri hiçbir kapı tarafından izlenmiyordu:** 09-01/15/22 incelemeleri fark edilmeden kaçırılmıştı. `check_policy_dates.py` artık her istisnanın `next_review` işaretini zorunlu tutuyor, yaklaşınca uyarıyor ve tarih geçince CI'ı fail-closed kırıyor; sert `expires` tarihi security gate için yedek olarak kalıyor.
- **pip-audit istisnaları 2026-10-01/08/13'te süresi dolup CI'ı kıracaktı:** istisnasız tarama yapıldı. Artık raporlanmayan `PYSEC-2026-597` (nltk) ve `CVE-2026-69112` (accelerate) kaldırıldı; düzeltmesi olmayan chromadb (3) ve nltk (1) istisnaları, erişilemez kod yolu koddan yeniden doğrulanarak 2026-12-31'e yenilendi.
- **npm audit istisnası 2026-09-30'da main'i kıracaktı:** istisna testi gerçek saatle `returncode == 0` bekliyordu. Advisory zinciri kaybolduğu için ölü `brace-expansion` allowlist'i planın çıkış ölçütüne göre kaldırıldı; audit artık istisnasız fail-closed. ESLint 10 incelemesi yapıldı: react/jsx-a11y eklentileri hâlâ desteklemiyor, sonraki tarih 2026-12-31.
...
- **Frontend TypeScript ratchet'i 2026-10-31 ara hedefini (en fazla 20 untyped dosya) kaçırmak üzereydi (21 untyped test dosyası):** `App.lazyFallback`, `App.chat-status`, `main`, `PanelErrorBoundary` ve `SwarmFlowPanel.helpers` testleri davranışları değiştirilmeden `.ts`/`.tsx`'e taşındı; baseline 16 untyped / 56 typed'a sıkılaştırıldı.
- **2026-09-30 tarihli üç Ruff politika tarihi dolunca `check_policy_dates.py` 1 Ekim'den itibaren her PR'da CI'ı kıracaktı:** Borcu zaten 0 olan E501/ASYNC240 incelemeleri kapatıldı. Kapanamayan 5.486 D100-D107 bulgusu tarih uzatılmak yerine `missing_docstring_debt_baseline` ratchet'ine bağlandı (yalnız azalabilir); ilerleme 2027-03-31'de gözden geçirilecek.
- **Canlı dokümanlardaki eskimiş dosya/yol referanslarını yakalayan bir kapı yoktu; #2876'da elle düzeltilen ~30 ölü referans sessizce birikmişti:** Yeni `scripts/ci/check_doc_links.py`, tarihsel kayıtlar hariç tüm takipli `*.md` dosyalarında kırık göreli Markdown bağlantılarını ve var olmayan repo yollarını CI'da ve `run_tests.sh` güvenlik kapısında sıfır toleransla reddediyor. Taşınmayı bilerek anlatan cümleler doküman bazlı `ALLOWED_MISSING` listesinde tutuluyor; artık eşleşmeyen istisnalar da hata sayılıyor.
