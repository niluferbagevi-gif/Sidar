...
* **Sağlayıcılar arası tool-calling şema farkları (`agent/tooling.py`, `core/llm_client.py`):** Şemalı araç argümanlarında legacy `|||` formatı kaldırılarak JSON/Pydantic tek yoluna geçildi; Sidar tool JSON formatı sağlayıcı-özel isteklere `build_provider_json_mode_config(...)` adaptörü ile dönüştürüldü.
* **Web UI (Vanilla JS) ölçeklenme riski giderimi (`web_ui/app.js`, `web_ui/chat.js`, `web_ui/sidebar.js`):** Merkezi `UIStore` mimarisine geçilerek state yönetimi toplulaştırıldı, DOM caching yardımcıları eklendi ve hata/olay yönetimi daha tutarlı hale getirildi.
