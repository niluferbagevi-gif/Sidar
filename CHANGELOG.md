...
- **`core.doctor`'daki `rag_index_ready`/`graphrag_entity_memory_ready`/`check_rag_readiness` kontrolleri, `database_env` "pass" derken pgvector'ü yanlışlıkla "blocked" işaretliyordu:** `check_database_env()` ve `check_database_connectivity()` DATABASE_URL'i her zaman paylaşılan `_resolved_database_urls()` çözücüsü üzerinden okur — bu çözücü, DATABASE_URL dotenv zincirinde açık tanımlı değilse (Sidar'ın kasıtlı tercih ettiği desen) onu POSTGRES_USER/PASSWORD/DB parçalarından türetir. `core/doctor/__init__.py` içindeki `_rag_readiness_state()` (pgvector dalı) ve geriye dönük uyumlu `check_rag_readiness()` ise doğrudan ham `os.getenv("DATABASE_URL", "")` okuyordu; DATABASE_URL yalnızca POSTGRES_* parçalarından türetildiğinde bu iki fonksiyon boş string görüp `database_env_status: "fail"` üretiyor ve `rag_index_ready`/`graphrag_entity_memory_ready`'yi `blocked_by: "database_env"` ile "warn" durumuna düşürüyordu — aynı çalıştırmada `database_env` kontrolü "pass" ve `pgvector_ready` kontrolü `select_1: true`/`pgvector_extension_installed: true` derken. Kullanıcıya önerilen `sync_database_passwords --remove-explicit-urls` auto-fix'i bu durumda hiçbir şeyi değiştirmiyordu çünkü bozuk olan DATABASE_URL değil, onu okuyan kontrol fonksiyonuydu. Her iki `os.getenv("DATABASE_URL", "")` çağrısı `_resolved_database_urls()[0]` ile değiştirildi (satır 19-23'teki `websocket_routes`/`database_connectivity`/`gpu_memory_config`/`redis`/`database_env` düzeltmeleriyle aynı "ham `os.getenv` vs. gerçek çözücü" deseni). `tests/unit/core/test_doctor.py`'a DATABASE_URL dotenv'de yokken yalnızca POSTGRES_*'ten türetildiği senaryoda `rag_index_ready`/`graphrag_entity_memory_ready`/`check_rag_readiness`'in artık yanlışlıkla "blocked" dönmediğini doğrulayan bir regresyon testi eklendi.
