...
from config import Config, get_sidar_int_env, get_sidar_optional_env
...
def _configured_ruff_unsafe_selectors() -> list[str]:
    env_value = get_sidar_optional_env("RUFF_AUTOFIX_UNSAFE_RULES", preserve_blank=True)
...
def build_local_failure_context(
    ...
    *,
    ...
    local_scope_limit = max(
        1,
        get_sidar_int_env("SELF_HEAL_LOCAL_SCOPE_LIMIT", Config.SELF_HEAL_LOCAL_SCOPE_LIMIT),
    )
...
def build_remediation_loop(context: dict[str, Any], diagnosis: str) -> dict[str, Any]:
    ...
    scope_hitl_threshold = max(
        1,
        get_sidar_int_env(
            "SELF_HEAL_HITL_SCOPE_THRESHOLD", Config.SELF_HEAL_HITL_SCOPE_THRESHOLD
        ),
    )
    auto_batch_size = max(
        1,
        get_sidar_int_env(
            "SELF_HEAL_AUTONOMOUS_BATCH_SIZE", Config.SELF_HEAL_AUTONOMOUS_BATCH_SIZE
        ),
    )
