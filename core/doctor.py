...
def check_database_connectivity() -> DoctorCheck:
    ...
    try:
        ...
        try:
            database_env_check = check_database_env()
        except Exception:  # pragma: no cover - defensive diagnostic path
            database_env_check = None
        if database_env_check is not None:
            database_env_status = str(getattr(database_env_check, "status", "") or "")
            details["database_env_status"] = database_env_status
            if (
                guidance.get("failure_category") == "authentication"
                and database_env_status == "pass"
            ):
                message = (
                    "PostgreSQL authentication failed after database_env parity passed; "
                    "the running PostgreSQL user likely still has an older stored password. "
                    "Run ALTER USER for the existing database user or reset the development volume. "
                    "Sidar will enter SQLite degraded mode and pgvector will fall back to BM25."
                )
                guidance["root_cause_hints"] = [
                    "database_env parity passed, so dotenv URL/password drift is unlikely",
                    "PostgreSQL Docker volume was initialized with an older password",
                    "sidar user exists with a different password in PostgreSQL",
                ]
                guidance["remediation_steps"] = [
                    "Run ALTER USER for the existing PostgreSQL user using the current POSTGRES_PASSWORD, or reset the PostgreSQL volume in development only.",
                    "Restart PostgreSQL and rerun `uv run python -m core.doctor artifacts/install/doctor.json`.",
                ]
