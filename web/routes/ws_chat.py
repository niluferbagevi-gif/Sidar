...
async def websocket_chat(websocket: WebSocket, deps: Any) -> Any:
    ...
    async def _cancel_task_and_wait(task: asyncio.Task[Any] | None) -> None:
        """Cancel an active task and, if possible, await it.

        This helper defensively checks whether the provided task is awaitable before
        awaiting it. In production, ``asyncio.Task`` instances are awaitable, but the
        unit tests may inject task-like stubs that lack an ``__await__`` method. Without
        this guard, awaiting such stubs would raise a TypeError. By checking
        ``inspect.isawaitable()``, we ensure compatibility with both real tasks and
        lightweight stubs.
        """
        ...
        with contextlib.suppress(asyncio.CancelledError):
            # Only await the task if it is awaitable. Test stubs may not implement __await__.
