...
function buildHandoffEvents(responseResults) {
...
function buildRoleHints(tasks, responseResults, handoffEvents) {
  return Array.from(
    new Set(
      [
...
function buildTelemetryWithActors(steps, roleHints) {
...
function buildLanes(roleHints, telemetryWithActors) {
...
function buildAutonomyNodes(autonomyItems, lanes) {
...
function buildSupervisorNode({ laneMap, mode, sessionId, maxConcurrency }) {
  return {
    id: "supervisor",
    type: "root",
    title: "Supervisor",
...
function buildTaskNodes(tasks, responseResults, laneMap) {
...
function buildAgentNodes(lanes) {
  return lanes
...
function buildHandoffNodes(handoffEvents, laneMap, responseResults) {
...
function buildResultNodes(responseResults, laneMap) {
...
function buildTelemetryNodes(telemetryWithActors, laneMap, laneDecisionCounts) {
...
function buildAutonomyEdges(autonomyNodes, supervisorNode) {
  const edges = [];
...
function buildTaskEdges(taskNodes, supervisorNode, mode) {
  const edges = [];
...
function buildResultEdges(resultNodes, responseResults, taskNodes, handoffNodes, supervisorNode, mode) {
  const edges = [];
...
function buildHandoffEdges(handoffNodes, handoffEvents, taskNodes, responseResults) {
  const edges = [];
...
function buildTelemetryEdges(telemetryNodes, resultNodes, responseResults) {
...
function computeGraphDimensions(lanes, laneDecisionCounts) {
...
function computeGraphMetrics(lanes, taskNodes, telemetryNodes, handoffNodes) {
  return {
    roles: lanes.length,
