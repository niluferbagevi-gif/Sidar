...
describe("useVoiceAssistant — advanced coverage paths", () => {
  function installVoiceEnv({
    token = "token",
    analyserFill = () => 128,
    audioPlayImpl,
  } = {}) {
    return import("../lib/api.js").then(({ getStoredToken }) => {
      getStoredToken.mockReturnValue(token);
      ...
      class MockAudioContext {
        ...
        createAnalyser() {
          return {
            fftSize: 2048,
            smoothingTimeConstant: 0.8,
            getByteTimeDomainData: (frame) => frame.fill(analyserFill()),
          };
        }
        close() { return Promise.resolve(); }
      }
      ...
      const play = audioPlayImpl || (() => Promise.resolve());
      const pause = vi.fn();
      ...
      return { ws, recorderInstanceRef: () => recorderInstance, rafRef: () => rafCallback };
    });
  }

  it("handles interruption/audio chunk/done branches and explicit interrupt cancel", async () => {
    const { ws } = await installVoiceEnv();
    const onAssistantDone = vi.fn();
    const onAssistantChunk = vi.fn();
    const onUserTranscript = vi.fn();
    const { result } = renderHook(() =>
      useVoiceAssistant({ onAssistantDone, onAssistantChunk, onUserTranscript, onError: vi.fn() }),
    );
    ...
    act(() => ws.onmessage?.({ data: JSON.stringify({ transcript: " merhaba " }) }));
    act(() => ws.onmessage?.({ data: JSON.stringify({ chunk: "cevap" }) }));
    act(() => ws.onmessage?.({ data: JSON.stringify({ audio_chunk: "AQID", audio_mime_type: "audio/wav" }) }));
    act(() => ws.onmessage?.({ data: JSON.stringify({ voice_interruption: "barge", cancelled_audio_sequences: 1 }) }));
    act(() => ws.onmessage?.({ data: JSON.stringify({ done: true }) }));

    expect(onUserTranscript).toHaveBeenCalledWith("merhaba");
    expect(onAssistantChunk).toHaveBeenCalledWith("cevap");
  ...
  it("covers requesting_permission label while microphone permission is pending", async () => {
    const { getStoredToken } = await import("../lib/api.js");
    getStoredToken.mockReturnValue("token");
    ...
    expect(result.current.statusLabel).toBe("İzin");
    await act(async () => {
      resolveMedia(stream);
      await Promise.resolve();
    });
  ...
  it("handles recorder arrayBuffer resolve after stop without sending append_base64", async () => {
    const { ws, recorderInstanceRef, rafRef } = await installVoiceEnv({
      analyserFill: (() => {
        let tick = 0;
        return () => {
          tick += 1;
          return tick === 1 ? 255 : 128;
        };
      })(),
    });
    ...
    act(() => ws.onmessage?.({ data: JSON.stringify({ auth_ok: true, voice_state: "processed" }) }));
    ...
    const delayedArrayBuffer = new Promise((resolve) => {
      resolveArrayBuffer = resolve;
    });

    const recorder = recorderInstanceRef();
    const onDataPromise = recorder.ondataavailable({
      data: { size: 8, arrayBuffer: () => delayedArrayBuffer },
    });
    ...
    await act(async () => {
      resolveArrayBuffer(new Uint8Array([9, 9, 9, 9]).buffer);
      await onDataPromise;
    });

    const actions = ws.send.mock.calls.map(([payload]) => JSON.parse(payload).action);
    expect(actions).not.toContain("append_base64");
  });

  it("flushes pending commit from recorder callback and exposes processing label", async () => {
    vi.useFakeTimers();
    const { ws, recorderInstanceRef, rafRef } = await installVoiceEnv({
      analyserFill: (() => {
        let tick = 0;
    ...
    act(() => ws.onmessage?.({ data: JSON.stringify({ auth_ok: true }) }));
    act(() => ws.onmessage?.({ data: JSON.stringify({ voice_state: "processed" }) }));
    ...
    const recorder = recorderInstanceRef();
    await act(async () => {
      await recorder.ondataavailable({
        data: {
          size: 10,
