...
- **Webhook / federation:** `ENABLE_EVENT_WEBHOOKS`, `GITHUB_WEBHOOK_REQUIRE_SIGNATURE`, `AUTONOMY_WEBHOOK_SECRET`, `AUTONOMY_WEBHOOK_REQUIRE_SIGNATURE`, `ENABLE_SWARM_FEDERATION`, `SWARM_FEDERATION_SHARED_SECRET`
- **GraphRAG:** `ENABLE_GRAPH_RAG`, `GRAPH_RAG_MAX_FILES`, `ENABLE_RAG_ENTITY_EXTRACTION`, `RAG_ENTITY_MAX_PER_DOC`
- **Cost-aware routing:** `ENABLE_COST_ROUTING`, `COST_ROUTING_COMPLEXITY_THRESHOLD`, `COST_ROUTING_LOCAL_PROVIDER`, `COST_ROUTING_LOCAL_MODEL`, `COST_ROUTING_CLOUD_PROVIDER`, `COST_ROUTING_CLOUD_MODEL`, `COST_ROUTING_DAILY_BUDGET_USD`, `COST_ROUTING_SHARED_BUDGET_DB_PATH`, `COST_ROUTING_REDIS_BUDGET_URL`, `COST_ROUTING_REDIS_BUDGET_NAMESPACE`
...
- Çoklu worker/pod kurulumlarında `COST_ROUTING_REDIS_BUDGET_URL` ve `COST_ROUTING_REDIS_BUDGET_NAMESPACE` aynı günlük bütçe sayacını Redis üzerinde paylaşır; namespace aynı Redis içinde production/staging çakışmasını önler.
