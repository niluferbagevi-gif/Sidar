...
- Repo’daki günlük baseline kalite geçidinin doğruluk kaynağı `pyproject.toml` içindeki `[tool.coverage.report].fail_under` değeridir; bu revizyonda değer `%5`'dır. CI ve campaign profilleri de açık override verilmedikçe bu ratchet edilmiş baseline'ı kullanır.
...
- Teknik kaynaklar (`pyproject.toml` + `run_tests.sh` + opsiyonel override'lar) tek kaynak olarak `pyproject.toml` baseline'ını ve açık ortam değişkenlerini kullanır: local/CI/campaign fail-under başlangıcı `%5` bootstrap gate'tir; coverage campaign'in `%100` hedefi merge-blocking gate değil ayrı iyileştirme hedefidir. Bu fazlar yeni modül ekleme veya risk-temelli test kampanyalarında kapasite planlama hedefi olarak kullanılmalıdır.
