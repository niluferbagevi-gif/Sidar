...
**v5.0 hedefi**, bu temeli bozmadan SİDAR'ı gerçek anlamda bir **AI Co-Worker** seviyesine çıkarmaktır. Bunun için beş ana eksende ilerleme önerilir:
...
5. **Otonom İş Ekosistemine Geçiş:** sadece kod yazan değil, test kapsamını otomatize eden (Coverage Ajanı), dijital pazarlama/operasyon yürüten (Poyraz Ajanı) ve kendi hatalarını otonom yamalayan (Auto-Healing) bir şirket altyapısı kurmak.
...
### 4.4 Set-of-Mark Tarayıcı Görüşü ve Doğrudan Platform Analizi

**Öneri:** `managers/browser_manager.py` ve ileride eklenecek görsel karar katmanı, klasik DOM seçicilerinin ötesine geçerek ekran görüntüsü üzerinde işaretlenmiş referans noktaları (Set-of-Mark) ile karar verebilmeli; ayrıca medya analizi yalnızca yerel MP4 yüklemeleriyle sınırlı kalmayıp YouTube gibi platformlardan doğrudan akış/metadata alabilecek şekilde genişletilmelidir.

#### Neden önemli?

- Modern web uygulamalarında DOM her zaman kararlı değildir; canvas, sanal liste ve anti-bot katmanlarında yalnızca selector tabanlı yaklaşım kırılgan kalır.
- Görsel referans noktalarıyla çalışan tarayıcı zekası, ajanların “hangi elementi neden tıkladığını” daha açıklanabilir hale getirir.
- Ürün demoları, eğitim videoları ve müşteri geri bildirimleri çoğu zaman YouTube gibi platformlarda yaşar; bu içerikleri doğrudan analiz edebilmek SİDAR'ın dış dünya algısını ciddi biçimde büyütür.

#### Mimari yaklaşım

- `capture_screenshot` + görsel işaretleme katmanı ile tıklanabilir bölgeleri Set-of-Mark koordinatlarına dönüştürmek.
- Browser aksiyonlarında gerektiğinde hibrit karar vermek: önce DOM/ARIA seçicileri, başarısız olursa görsel işaretler ve ekran bölgesi eşleştirmesi.
- `core/multimodal.py` veya yeni medya bağlayıcıları üzerinden YouTube/uzak platform akışlarını geçici frame + transcript bağlamına dönüştürmek.

#### Minimum uygulanabilir kapsam (MVP)

1. Tarayıcı ekran görüntüsüne işaretlenmiş referans numaraları bas.
2. LLM'e bu işaretler üzerinden “hangi alan tıklanmalı” kararı aldır.
3. YouTube URL'si için metadata + transcript/frame özetleme hattı kur.
4. DOM tabanlı aksiyon ile görsel aksiyon arasında denetlenebilir fallback kaydı tut.

#### Riskler

- Görsel işaretleme latency ekleyebilir.
- Platform API kotaları ve telif/politika sınırları dikkatle yönetilmelidir.
- Görsel kararlar için güçlü audit trail üretilmezse yanlış tıklamaların açıklanabilirliği düşer.
...
### 6.3 Coverage Ajanı (Test Uzmanı)

**Öneri:** `managers/code_manager.py` üzerinden coverage raporlarını, başarısız testleri ve edge-case boşluklarını okuyan; `%99.9` test kapsamını korumak için sürekli otonom test üreten ayrı bir uzman rol tanımlanmalıdır.

#### Rol tanımı

- Coverage raporlarını (`coverage.xml`, HTML summary, CI artifact'ları) okur.
- Düşük kapsamlı satırları ve dalları test görevi olarak parçalar.
- Coder/Reviewer zinciriyle çakışmadan yalnızca test dosyaları ve kalite artifact'larında çalışır.
- Kritik modüllerde kapsam düşüşü gördüğünde proaktif görev başlatabilir.

#### Mimari etkiler

- `agent/core/supervisor.py` içinde yeni intent/role routing sinyali gerekir.
- `agent/tooling.py` tarafında `read_coverage_report`, `generate_missing_tests`, `run_targeted_tests` gibi kalite odaklı araçlar tanımlanmalıdır.
- `managers/code_manager.py` coverage parse etme ve test senaryosu önerme yetenekleriyle genişlemelidir.

### 6.4 Poyraz Ajanı (Dijital Pazarlama ve Operasyon Uzmanı)

**Öneri:** SİDAR'ın dışa dönük ticari/operasyonel yüzünü temsil edecek `PoyrazAgent`, web sitesi taslakları üretme, kampanya içeriği planlama ve Instagram/Facebook/WhatsApp gibi kanallara bağlanan operasyon araçlarını yönetebilmelidir.

#### Sorumluluk alanı

- Açılış sayfası, kampanya metni ve ürün anlatımı üretimi
- Sosyal medya içerik takvimi ve gönderi taslakları
- WhatsApp/Instagram/Facebook entegrasyonları üzerinden müşteri iletişimi
- Platform metriklerini okuyup hangi mesajların dönüşüm getirdiğini raporlama

#### Mimari etkiler

- Yeni araç grubu: medya üretimi, sosyal paylaşım, müşteri yanıt şablonları
- `agent/swarm.py` içinde iç operasyon ajanları ile dış iletişim ajanları arasında güvenli delege sınırları
- HITL + audit katmanının marka, hukuk ve müşteri iletişimi risklerine göre genişletilmesi

### 6.5 Otonom Kendi Kendini İyileştirme (Auto-Healing)

**Öneri:** Docker loglarını, web sunucu istisnalarını ve çalışma zamanı telemetrisini dinleyen; GraphRAG ile kök neden analizi yapıp otonom yama öneren kapalı döngü bir auto-healing hattı tanımlanmalıdır.

#### Döngü

1. Log listener hata/uyarıyı yakalar.
2. `core/rag.py` içindeki GraphRAG bağımlılık grafiği ile etki alanı çıkarılır.
3. Coder/Reviewer/Coverage zinciri önerilen düzeltmeyi doğrular.
4. Güvenli ise patch, test ve deploy hazırlığı otomatikleştirilir; riskliyse HITL'e düşer.

#### Başarı ölçütleri

- Tekrarlayan operasyonel hatalarda MTTR düşüşü
- Sıfır kesinti hedefi için rollback + patch önerilerinin audit edilebilir olması
- Log → analiz → patch → test döngüsünün tek telemetry izinde izlenebilmesi
...
### Faz D — Otonom İş Ekosistemi ve Auto-Healing

1. Coverage Ajanı entegrasyonu (%99.9 tam otonom test döngüsü)
2. Poyraz Ajanı ve sosyal medya / platform API entegrasyonları
3. YouTube doğrudan analiz ve Set-of-Mark tarayıcı navigasyonu
4. Otonom Auto-Healing (sıfır kesinti) kapalı döngüsü
...
| P0 | Coverage Ajanı iskeleti | `agent/roles/coverage_agent.py`, `managers/code_manager.py` | Otonom coverage okuma + test üretim döngüsü |
| P0 | Poyraz Ajanı ve tooling tanımları | `agent/roles/poyraz_agent.py`, `agent/tooling.py`, `managers/social_manager.py` | Dijital operasyon araç yüzeyi |
| P1 | YouTube API ve Set-of-Mark Vision | `core/multimodal.py`, `managers/browser_manager.py` | Platformdan doğrudan medya analizi + görsel navigasyon |
| P2 | Auto-Healing log listener | `web_server.py`, `core/rag.py`, yeni `managers/log_listener.py` | Logdan patch öneren kapalı döngü |
