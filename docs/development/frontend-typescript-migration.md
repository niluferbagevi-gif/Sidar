...
52 untyped / 8 typed seviyesine ilerletmiştir.
...
`typescript-migration-baseline.json`, en fazla 52 untyped (`.js` + `.jsx`) ve en az
8 typed (`.ts` + `.tsx`) kaynak dosyasına izin verir. `npm run typecheck:inventory`:
