...
├── <a href="pyproject.toml</a>                # Coverage kalite kapısı kuralları (%5 eşik)
...
- **Kapsama politikası:** `pyproject.toml`, `pytest.ini`, `run_tests.sh` ve CI hattı ile yönetilen **%5 hard gate**
...
| Coverage Quality Gate (local/CI `fail_under=5` bootstrap, açık override/campaign opt-in) | ✅ Zorunlu | `pyproject.toml`, `run_tests.sh`, `.github/workflows/ci.yml`, `AGENTS.md §2.5.4` |
...
- **CI profile (`TEST_PROFILE=ci`)** — `.github/workflows/ci.yml` ana test job'unda varsayılan olarak aynı
  ratchet baseline'ı kullanır; geçici sıkılaştırma gerekiyorsa
  `COVERAGE_FAIL_UNDER_CI` açıkça verilir.
- **Coverage campaign** — `COVERAGE_CAMPAIGN=1` veya
  `AUTONOMOUS_LOOP_OPERATION_PROFILE=coverage-campaign` ile devreye girer,
  `COVERAGE_FAIL_UNDER_CAMPAIGN` verilmezse aynı ratchet baseline
  uygulanır; ayrı otonom hedef/ratchet üst sınırı campaign profilinde yönetilir.
...
| **Sıfır Borç Kalite Kapısı** | Agresif test envanteri, CI kalite kapıları ve `%5` coverage hard gate (`.github/workflows/ci.yml`, `run_tests.sh`, `pyproject.toml`, `tests/`) | ✅ Tamamlandı |
...
> **Not:** “%100 test kapsaması” ifadesi kültürel/aspirasyonel hedef olarak korunur ve coverage campaign profilinde (`COVERAGE_CAMPAIGN=1` veya `AUTONOMOUS_LOOP_OPERATION_PROFILE=coverage-campaign`) ayrı opt-in hedef olarak ele alınır. Günlük local ve CI kalite kapısı `pyproject.toml` üzerinde `%5` bootstrap tabanını kullanır; daha sıkı `COVERAGE_FAIL_UNDER_CI`/`COVERAGE_FAIL_UNDER_CAMPAIGN` değerleri yalnız açık override ile uygulanır; üç profil tek bir koddan değil farklı operasyonel hedeflerden beslenir (bkz. `AGENTS.md §2.5.4`).
