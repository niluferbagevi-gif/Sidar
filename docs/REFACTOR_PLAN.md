...
|---|---:|---|
| `web_server.py` | 3289 | Web/API kontrol düzlemi için route/middleware/lifecycle wrapper azaltımı sürmeli. |
| `run_tests.sh` | 2857 | Quality gate orchestration büyüdü; benchmark/frontend/coverage stage helper'ları ayrı script modüllerine çıkarılmalı. |
| `install_sidar.sh` | 2040 | `scripts/install_modules/` yapısı var; ana script bootstrap/UX facade olarak inceltilmeli. |
| `config.py` | 1860 | Dotenv helperları ayrıldı; secret/runtime path/logging bootstrap boundary'leri daraltılmalı. |
| `managers/code_manager.py` | 1706 | Docker/patch/security helperları ayrıldı; sandbox lifecycle adapter'ı sonraki güvenli adım. |
| `agent/sidar_agent.py` | 1684 | Self-heal/autonomy/federation servisleri ayrıştırılarak facade küçültülmeli. |

## Öncelikli büyük dosyalar

| Dosya | Risk nedeni | Hedef modül sınırı | İlk düşük riskli adım | Test kapısı |
|---|---|---|---|---|
| `web_server.py` | Endpoint, WebSocket, middleware, auth, frontend fallback ve bootstrap tek dosyada yoğunlaşıyor. | `web/routes/ws_chat.py`, `web/routes/ws_voice.py`, `web/routes/collaboration.py`, `web/routes/webhooks.py`, `web/routes/operations.py`, `web/security.py`, `web/app_factory.py`, `web/bootstrap.py`, `web/middleware/cors.py` | App factory boundary'si `web/app_factory.py` içine çıkarıldı; GitHub webhook router çıkarımı yapıldı; `/ws/chat` router factory, `/ws/voice` router factory, HITL router ve WebSocket token parser (`web/security.py`) çıkarımı yapıldı; frontend static mount ve SPA fallback bootstrap boundary'si `web/bootstrap.py` içine çıkarıldı; **middleware/frontend fallback bootstrap boundary** kapsamında loopback CORS middleware bootstrap'ı `web/middleware/cors.py`, frontend fallback/bootstrap akışı `web/bootstrap.py` modüllerine taşındı; sıradaki düşük riskli adım middleware/lifecycle bootstrap boundary'sini daraltmak ve kalan legacy wrapper sayısını azaltmak. | `tests/unit/root/test_web_server.py`, `tests/unit/web/*`, smoke boot tests |
| `run_tests.sh` | Static analysis, coverage, BATS, benchmark, frontend, artifact summary ve production-readiness orchestration tek shell dosyasında büyüyor. | `scripts/test_gates/static_analysis.sh`, `scripts/test_gates/backend_pytest.sh`, `scripts/test_gates/benchmark.sh`, `scripts/test_gates/frontend.sh`, `scripts/test_gates/summary.py` | Önce salt raporlama/summary JSON üretimini Python helper'a çıkarıp `run_tests.sh` içinde komut uyumluluğu ve env contract'ını korumak. | `make production-readiness`, `tests/unit/scripts/*`, shellcheck/BATS gates |
| `install_sidar.sh` | Modüler installer modülleri olmasına rağmen bootstrap, UX, validation ve fallback yüzeyi ana scriptte yoğun kalıyor. | `scripts/install_modules/bootstrap.sh`, `scripts/install_modules/ux.sh`, `scripts/install_modules/validation.sh`, `scripts/install_modules/offline_bundle.sh` | Ana scripti yalnız argument parsing + phase dispatcher facade'ına indirmek; mevcut `scripts/install_modules/` phase sözleşmesini bozmadan validation çıktısını modüle taşımak. | installer smoke, `tests/unit/scripts/*`, shellcheck |
