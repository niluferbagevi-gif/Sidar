...
| `core/db/monolith.py` | 1259 | Eski `core/db.py` artık `core/db/` paketi; auth/audit/session/user/prompt/metrics/diagnostics/records/dialect/helpers/marketing/access-policy modülleri ayrıldı ve `core/db/__init__.py` geriye dönük uyumlu facade oldu. Kota/budget yazma gövdesi dahil provider usage akışları `metrics.py`, mesaj CRUD `sessions.py`, access-policy list/upsert/check akışı `access_policy.py` içindedir; schema bootstrap yüzeyi hâlâ `monolith.py` içinde yoğunlaşıyor. |
