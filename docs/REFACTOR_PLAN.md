...
|---|---:|---|
| `web_server.py` | 2625 | Auth/JWT ile webhook HMAC/replay guard `web/security.py`, access-policy eşleme/serileştirme `web/middleware/access_policy.py`, rate-limit `web/middleware/ratelimit.py`, plugin policy `web/plugins/sandbox.py`, request modelleri `web/routes/request_models.py`, ortak prompt/swarm serileştirme `web/routes/serialization.py` ve route domainleri `web/routes/` altında ayrıldı; plugin sandbox policy üretimde fail-closed kalır, ana dosyada legacy monkeypatch/import wrapper azaltımı sürmeli. |
...
| `core/db/monolith.py` | 848 | Eski `core/db.py` artık `core/db/` paketi; auth/audit/session/user/prompt/metrics/diagnostics/records/dialect/helpers/marketing/access-policy/schema modülleri ayrıldı ve `core/db/__init__.py` geriye dönük uyumlu facade oldu. Kota/budget yazma gövdesi dahil provider usage akışları `metrics.py`, mesaj CRUD `sessions.py`, access-policy list/upsert/check akışı `access_policy.py`, SQLite bootstrap DDL'i ile Alembic tabanlı PostgreSQL şeması `schema.py` içindedir; `monolith.py` artık bu domain modüllerine delege eden ince wrapper'lar ve compatibility facade yüzeyinden ibaret. |
| `config.py` | 1790 | Dotenv, logging, secret hardening, runtime path inference, rate-limit/Redis, event bus/Kafka, RAG store, Docker sandbox, pazarlama/sosyal entegrasyon, GitHub/Hugging Face, web arama, cost-aware routing, LoRA/QLoRA, sürekli öğrenme (continuous learning), multimodal vision/ses, tarayıcı otomasyonu, LSP entegrasyonu, otonomi servisi webhook, RAG LLM entity extraction, web arayüzü bind ve bellek şifrelemesi loaderları ayrıldı; facade hâlâ çok sayıda class attribute compatibility yüzeyi taşıyor. |
...
| `core/db/monolith.py` (eski `core/db.py`) | Compatibility facade yüzeyi hâlâ tek ~848 satırlık modülde; şema/domain davranışı artık alt modüllere delege ediliyor. | `core/db/auth.py`, `core/db/audit.py`, `core/db/coverage.py`, `core/db/marketing.py`, `core/db/access_policy.py`, `core/db/schema.py`, `core/db/session.py`, ortak `core/db/models.py` | `core/db.py` artık `core/db/` paketi; `auth.py`, `audit.py`, `coverage.py`, `marketing.py`, `session.py`, `sessions.py`, `users.py`, `prompt_registry.py`, `metrics.py` (provider usage, quota/budget ve admin istatistikleri) ve `models.py` akışları çıkarıldı, `audit_log.py` legacy compatibility wrapper olarak bırakıldı. PostgreSQL teşhisleri `core/db/diagnostics.py`, saf zaman/JSON/UUID/SQLite helperları `core/db/helpers.py`, SQL dialect/asyncpg tag helperları `core/db/dialect.py`, domain dataclass kayıtları `core/db/records.py` modüllerine taşındı ve `core/db/__init__.py` legacy re-export facade'ı olarak korundu. 2026-07-20'de ilk read-only query helper adımı atıldı: `list_marketing_campaigns`, `list_content_assets`, `list_operation_checklists` gövdeleri `core/db/marketing.py`'ye taşındı ve `Database` üzerindeki metotlar ince delege wrapper'a indirildi (`core/db/__init__.py`'deki submodule whitelist'ine `marketing` eklendi — aksi halde `core.db` self-aliasing swap'ı sonrası `import core.db.marketing` kırılıyordu). 2026-08-02 itibarıyla `upsert_user_quota` gövdesi de `metrics.py` sınırına taşındı; facade yalnız delege eder. `list_access_policies`/`upsert_access_policy`/`check_access_policy` gövdeleri `core/db/access_policy.py`'ye taşındı ve `Database` üzerindeki üç metot da aynı ince delege wrapper deseniyle indirildi (`check_access_policy` içindeki iç `list_access_policies` çağrısı hâlâ `db.list_access_policies(...)` üzerinden `self`'e gidiyor, böylece mevcut instance-level monkeypatch davranışı korunuyor). SQLite/Alembic schema bootstrap akışı da `core/db/schema.py`'ye taşındı: `init_schema`, `_ensure_access_control_schema_{sqlite,postgresql}`, `_ensure_audit_log_schema_{sqlite,postgresql}`, `_init_schema_sqlite`, `_run_alembic_upgrade_head`, `_init_schema_postgresql`, `_ensure_schema_version_{sqlite,postgresql}` — hepsi `Database` üzerinde aynı isimli ince delege wrapper olarak kaldı; `schema.py` içindeki her adımlar-arası çağrı (`init_schema`'nın alt adımları, `_init_schema_postgresql`'in `_run_alembic_upgrade_head`'i çağırması gibi) modül-içi fonksiyona değil `db.X(...)` üzerinden `self`'e gidiyor, böylece `tests/unit/core/test_db.py::test_postgresql_schema_helpers_and_init_routing` gibi instance-level monkeypatch testleri (`db._init_schema_postgresql = lambda: ...` vb.) davranışını bire bir korudu. **Nüans (bir arkadaş kod incelemesi `_init_schema_sqlite()`/`_init_schema_postgresql()` arasındaki drift riskini ayrıca işaret etti):** `_init_schema_sqlite()` (14 `CREATE TABLE`'lık elle yazılmış tek SQL string'i) ile `_init_schema_postgresql()` (Alembic migration'larını çalıştırır) bağımsız, elle bakım gören iki kaynak olduğu için yeni bir Alembic migration'ının SQLite string'ine yansıtılmaması hiçbir testle yakalanmıyordu -- bu risk bu incelemenin erken bir maddesinde zaten kapatıldı: `tests/integration/db/test_db_migrations_integration.py::test_sqlite_bootstrap_schema_matches_alembic_head_schema` iki şemanın tablo/sütun kümesini VE `nullable`/`default` değerlerini karşılaştırıyor (backend-tipi temsil farklarını normalize ederek) ve CI'nın `integration` aşamasında (`run_tests.sh --stage all`) çalışıyor -- yani önerilen (b) seçeneği (karşılaştırma CI kontrolü) zaten uygulanmış durumda; (a) seçeneği (SQLite şemasını doğrudan Alembic'ten üretmek) hâlâ mimari bir seçim olarak açık kalıyor, bu taşıma yalnızca dosya sınırını değiştirdi, iki-kaynaklı tasarımı değiştirmedi. Sıradaki düşük riskli adım artık `core/db/monolith.py`'de kalan `ensure_user`/session/message CRUD delege yüzeyini daha da inceltmek veya `config.py`'nin kalan `GITHUB_*`/`HF_TOKEN` alan grubunu taşımaktır. | `tests/unit/core/test_db.py`, `tests/integration/db/test_db_migrations_integration.py::test_sqlite_bootstrap_schema_matches_alembic_head_schema` |
...
| `config.py` | Env yükleme, secret hardening, GPU detect orkestrasyonu, runtime path inference, logging bootstrap ve çok sayıda backend domain setting'i tek facade içinde yoğunlaşıyor. | `core/config_dotenv.py`, `core/config_dirs.py`, `core/config_gpu_detect.py`, `core/config_logging_setup.py`, `core/config_runtime_env.py`, `core/config_runtime_paths.py`, `core/config_secret_hardening.py`, `core/config_secrets.py`, `core/config_rate_limit.py`, `core/config_event_bus.py`, `core/config_rag_store.py`, `core/config_sandbox.py`, `core/config_social_integrations.py`, `core/config_github_hf.py`, `core/config_web_search.py`, `core/config_cost_routing.py`, `core/config_lora_training.py`, `core/config_continuous_learning.py`, `core/config_multimodal.py`, `core/config_browser.py`, `core/config_lsp.py`, `core/config_autonomy.py`, `core/config_rag_entity.py`, `core/config_web_server.py`, `core/config_memory_security.py` | Dotenv parse/load/reload tanılama helperları `core/config_dotenv.py`, logging bootstrap helperları `core/config_logging_setup.py`, secret hardening wrapperları `core/config_secret_hardening.py`, runtime path inference helperları `core/config_runtime_paths.py`, rate-limit/Redis loader'ı `core/config_rate_limit.py`, event bus/Kafka/RabbitMQ loader'ı `core/config_event_bus.py`, RAG store/pgvector loader'ı `core/config_rag_store.py` ve Docker sandbox loader'ı `core/config_sandbox.py` içine çıkarıldı; statik RAG default modülü `config_rag_defaults.py` olarak netleştirildi ve legacy `config_rag.py` yalnız shim olarak bırakıldı. Güncel değerlendirme: `config.py` gerçek bir god object değil; geriye dönük uyum için yaklaşık yirmi domain setting modülünü birleştiren compatibility facade'dır. İlk düşük riskli adım olarak `Config.llm_settings`, `Config.security_settings`, `Config.sandbox_settings` ve benzeri typed domain settings objeleri doğrudan expose edildi; sıradaki adım yeni kodu bu objelere yönlendirip legacy `Config.FOO` class attribute patch sayısını azaltmak. **Nüans:** bir arkadaş kod incelemesinde tespit edildiği üzere, `Config` sınıf gövdesinde tipli domain settings'e delege eden alanlarla (`AI_PROVIDER: str = LLM_SETTINGS.AI_PROVIDER` gibi) hâlâ ham `os.getenv(...)` inline çözümlenen ~35 alan yan yana duruyordu — bir okuyucu tek tek bakmadan hangi `Config.X` alanının tipli mi ad-hoc mu çözümlendiğini öngöremiyordu. Bunların hepsini tek seferde typed settings objelerine taşımak (10+ yeni domain modülü) bu incelemenin diğer maddeleriyle aynı düşük-risk/küçük-PR disiplinine uymadığından tek turda yapılmadı; en izole grup olarak işaretlenen pazarlama/sosyal entegrasyon alanları (`META_GRAPH_*`/`INSTAGRAM_*`/`FACEBOOK_*`/`WHATSAPP_*`/`SLACK_*`/`JIRA_*`/`TEAMS_WEBHOOK_URL`, 15 alan) `core/config_social_integrations.py`'deki `SocialIntegrationSettings`/`load_social_integration_settings()` içine taşındı ve `Config.social_integration_settings` olarak da expose edildi; legacy `Config.FOO` attribute isimleri/değerleri ve `JIRA_BASE_URL`/`JIRA_API_TOKEN` geriye-dönük alias fallback davranışı birebir korundu. GitHub/Hugging Face entegrasyon alanları (`GITHUB_TOKEN`/`GITHUB_REPO`/`GITHUB_WEBHOOK_SECRET`/`GITHUB_WEBHOOK_REQUIRE_SIGNATURE`/`HF_TOKEN`/`HF_HUB_OFFLINE`/`HF_USE_LOCAL_CACHE_ONLY`, 7 alan) aynı desenle `core/config_github_hf.py`'deki `GitHubHuggingFaceSettings`/`load_github_huggingface_settings()` içine taşındı ve `Config.github_huggingface_settings` olarak da expose edildi; `GITHUB_WEBHOOK_REQUIRE_SIGNATURE`/`HF_USE_LOCAL_CACHE_ONLY` katı `get_bool_env()`, `HF_HUB_OFFLINE` ise üçüncü parti kütüphane konvansiyonuna uygun `get_external_bool_env()` kullanmaya devam ediyor — bu ayrım birebir korundu. Web arama alanları (`SEARCH_ENGINE`/`TAVILY_API_KEY`/`GOOGLE_SEARCH_API_KEY`/`GOOGLE_SEARCH_CX`/`WEB_SEARCH_MAX_RESULTS`/`WEB_FETCH_TIMEOUT`, 6 alan) aynı desenle `core/config_web_search.py`'deki `WebSearchSettings`/`load_web_search_settings()` içine taşındı ve `Config.web_search_settings` olarak da expose edildi (`WEB_FETCH_MAX_CHARS`/`WEB_SCRAPE_MAX_CHARS` zaten önceden `core/config_rag_store.py`'ye taşınmıştı, bu dilimin kapsamı dışında bırakıldı). Cost-aware model routing alanları (`ENABLE_COST_ROUTING` + `COST_ROUTING_*`, 11 alan) aynı desenle `core/config_cost_routing.py`'deki `CostRoutingSettings`/`load_cost_routing_settings()` içine taşındı ve `Config.cost_routing_settings` olarak da expose edildi. **Düzeltme:** bu satırın önceki sürümü `DB_DEGRADED_SQLITE_URL`/`DB_SCHEMA_VERSION_TABLE`'ı "hâlâ inline" listesinde tutuyordu; bu iki alan zaten `Config.database_settings`/`_DATABASE_SETTINGS`'e delege edilmiş durumda, listeden çıkarıldı. LoRA/QLoRA fine-tuning alanları (`ENABLE_LORA_TRAINING` + 8 `LORA_*` alan, 9 alan) aynı desenle `core/config_lora_training.py`'deki `LoraTrainingSettings`/`load_lora_training_settings()` içine taşındı ve `Config.lora_training_settings` olarak da expose edildi (`core/active_learning.py`'nin ayrıca okuduğu `LORA_MODEL_REVISION`, `config.py`'de hiç tanımlı olmadığı için kapsam dışı bırakıldı; yalnız opsiyonel runtime `getattr` yüzeyi). Sürekli öğrenme (continuous learning) bundle alanları (`ENABLE_CONTINUOUS_LEARNING` + 6 `CONTINUOUS_LEARNING_*` alan, 7 alan) aynı desenle `core/config_continuous_learning.py`'deki `ContinuousLearningSettings`/`load_continuous_learning_settings()` içine taşındı ve `Config.continuous_learning_settings` olarak da expose edildi; `core/active_learning.py`'nin bu alanları mevcut `Config.FOO` adlarıyla `getattr` üzerinden tüketmesi birebir korundu. Multimodal vision/ses alanları (`ENABLE_VISION`/`VISION_MAX_IMAGE_BYTES`/`ENABLE_MULTIMODAL`/`MULTIMODAL_MAX_FILE_BYTES` + 11 `VOICE_*`/`WHISPER_MODEL` alan, 15 alan; `config.py`'deki `# ─── Multimodal Vision (v6.0) ───` bloğunun tamamı) aynı desenle `core/config_multimodal.py`'deki `MultimodalSettings`/`load_multimodal_settings()` içine taşındı ve `Config.multimodal_settings` olarak da expose edildi; `AGENTS.md` §2.7'de dokümante edilen feature-flag sözleşmesi ve `core/vision.py`/`core/multimodal.py`/`core/voice.py`/`web/routes/ws_voice.py`'nin `getattr(config, "FOO", default)` tüketimi birebir korundu (`core/voice.py`'nin ayrıca okuduğu `VOICE_ENABLED`, `config.py`'de hiç tanımlı olmadığı için kapsam dışı bırakıldı; genel websocket kimlik doğrulama zaman aşımı `WS_AUTH_TIMEOUT_SECONDS` da multimodal'e özgü olmadığından kapsam dışı bırakıldı). Tarayıcı otomasyonu alanları (`BROWSER_PROVIDER`/`BROWSER_HEADLESS`/`BROWSER_TIMEOUT_MS`/`BROWSER_ALLOWED_DOMAINS`, 4 alan) aynı desenle `core/config_browser.py`'deki `BrowserSettings`/`load_browser_settings()` içine taşındı ve `Config.browser_settings` olarak da expose edildi; `managers/browser_manager.py`'nin `getattr(self.cfg, "BROWSER_PROVIDER", "auto")` gibi mevcut alan adlarıyla tüketmesi birebir korundu. LSP entegrasyon alanları (`ENABLE_LSP`/`LSP_TIMEOUT_SECONDS`/`LSP_MAX_REFERENCES`/`PYTHON_LSP_SERVER`/`TYPESCRIPT_LSP_SERVER`, 5 alan) aynı desenle `core/config_lsp.py`'deki `LspSettings`/`load_lsp_settings()` içine taşındı ve `Config.lsp_settings` olarak da expose edildi; `managers/code_manager.py`'nin `getattr(self.cfg, "ENABLE_LSP", True)` gibi mevcut alan adlarıyla tüketmesi birebir korundu. Otonomi servisi webhook alanları (`AUTONOMY_SERVICE_USER_ID`/`AUTONOMY_WEBHOOK_SECRET`/`AUTONOMY_WEBHOOK_REQUIRE_SIGNATURE`, 3 alan) aynı desenle `core/config_autonomy.py`'deki `AutonomySettings`/`load_autonomy_settings()` içine taşındı ve `Config.autonomy_settings` olarak da expose edildi; `SYSTEM_USER_ID`/`SIDAR_AUTONOMY_WEBHOOK_SECRET`/`SIDAR_AUTONOMY_WEBHOOK_REQUIRE_SIGNATURE` legacy alias fallback davranışı ve `web/autonomy_bridge.py`/`web/routes/autonomy.py`'nin mevcut alan adlarıyla tüketmesi birebir korundu. RAG LLM entity extraction alanları (`ENABLE_RAG_LLM_ENTITY_EXTRACTION`/`RAG_LLM_ENTITY_PROVIDER`/`RAG_LLM_ENTITY_MODEL`/`RAG_LLM_ENTITY_REVIEW_TARGET`, 4 alan) aynı desenle `core/config_rag_entity.py`'deki `RagEntitySettings`/`load_rag_entity_settings()` içine taşındı ve `Config.rag_entity_settings` olarak da expose edildi; `core/rag/__init__.py`'nin `getattr(self.cfg, "ENABLE_RAG_LLM_ENTITY_EXTRACTION", False)` gibi mevcut alan adlarıyla tüketmesi birebir korundu (genel non-LLM `ENABLE_RAG_ENTITY_EXTRACTION`/`RAG_ENTITY_MAX_PER_DOC` ve GraphRAG `ENABLE_GRAPH_RAG`/`GRAPH_RAG_MAX_FILES` alanları farklı bir kapsam olduğundan bu dilimin dışında bırakıldı). Web arayüzü bind alanları (`WEB_HOST`/`WEB_PORT`/`WEB_GPU_PORT`, 3 alan; önceki sürümde yalnız `WEB_HOST` "kalan" listesinde işaretlenmişti, `WEB_PORT`/`WEB_GPU_PORT`'un da aynı `# ─── Web Arayüzü ───` bloğunda hâlâ inline olduğu bu dilimde fark edildi) aynı desenle `core/config_web_server.py`'deki `WebServerSettings`/`load_web_server_settings()` içine taşındı ve `Config.web_server_settings` olarak da expose edildi; `core/config_runtime_env.py::apply_runtime_env_overrides(...)`'in reload sırasında `Config.WEB_HOST`/`Config.WEB_PORT` class attribute'larını doğrudan yeniden ataması (zaten aynı desende delege edilmiş `CODING_MODEL`/`AI_PROVIDER` gibi diğer reload-sensitive alanlarla tutarlı biçimde) ve `main.py`/`web/cli.py`/`launcher/selection.py`'nin mevcut alan adlarıyla tüketmesi birebir korundu. Bellek şifrelemesi alanları (`MEMORY_ENCRYPTION_KEY`/`MEMORY_ENCRYPTION_KEY_PREVIOUS`, 2 alan) aynı desenle `core/config_memory_security.py`'deki `MemorySecuritySettings`/`load_memory_security_settings()` içine taşındı ve `Config.memory_security_settings` olarak da expose edildi; `Config.validate_critical_settings(...)`'in `cls.MEMORY_ENCRYPTION_KEY`/`cls.MEMORY_ENCRYPTION_KEY_PREVIOUS` tabanlı Fernet ön doğrulaması, `core/config_secret_hardening.py`'nin production zorunluluğu kontrolü ve `agent/sidar_agent.py`'nin mevcut alan adlarıyla tüketmesi birebir korundu. Kalan iki bağımsız tekil alan (`TEXT_MODEL`, `REVIEWER_TEST_COMMAND`) hâlâ inline; bunlar kendi başlarına ayrı bir domain modülü oluşturmaya değecek kadar büyük/cohesive bir grup oluşturmuyor (`TEXT_MODEL` zaten `config_llm.py`'nin `LLMClientSettings`'ine ait olabilecek bir aday ama bu dosya bu REFACTOR_PLAN.md maddesinin kapsamı dışında; `REVIEWER_TEST_COMMAND` tek başına kalan tek bir multi-agent geçiş alanı), bu nedenle aynı desenle taşınıp taşınmayacağı ayrı bir değerlendirme konusudur. | `tests/unit/root/test_config.py`, `tests/unit/core/test_config_social_integrations.py`, `tests/unit/core/test_config_github_hf.py`, `tests/unit/core/test_config_web_search.py`, `tests/unit/core/test_config_cost_routing.py`, `tests/unit/core/test_config_lora_training.py`, `tests/unit/core/test_config_continuous_learning.py`, `tests/unit/core/test_config_multimodal.py`, `tests/unit/core/test_config_browser.py`, `tests/unit/core/test_config_lsp.py`, `tests/unit/core/test_config_autonomy.py`, `tests/unit/core/test_config_rag_entity.py`, `tests/unit/core/test_config_web_server.py`, `tests/unit/core/test_config_memory_security.py`, boot diagnostics tests |
...
| `core/doctor/__init__.py` (eski `core/doctor.py`; repo'nun en büyük dosyası, 862 satır) | Health check, dependency probing, GPU/Redis/DB/RAG doğrulamaları ve orchestration tek facade içinde yoğunlaşıyor. | `core/doctor/checks/database.py`, `core/doctor/checks/gpu.py`, `core/doctor/checks/redis.py`, `core/doctor/checks/rag.py`, `core/doctor/checks/security.py`, `core/doctor/reporting.py`, `core/doctor/models.py`, `core/doctor/facade.py` | Yan etkisiz check result modeli/contract/redaction helperları `core/doctor/models.py`, rapor aggregate/write formatlayıcıları `core/doctor/reporting.py` içine çıkarıldı ve `core.doctor` legacy facade exportları korundu. **Nüans:** `core/doctor/checks/*.py` dosyaları zaten var, ama bir arkadaş kod incelemesinde tespit edildiği üzere gerçek ayrıştırma değildi — 5 dosyadaki 10 fonksiyondan 9'u (`database.py`'nin 3'ü, `gpu.py`'nin 2'si, `rag.py`'nin 3'ü, `security.py`'nin 1'i) tek satırlık pass-through (`return _doctor.check_x()`, gerçek gövde hâlâ `__init__.py`'deydi); yalnızca `redis.py::check_redis` gerçekten kendi kendine yeten, `__init__.py`'de karşılığı olmayan bağımsız bir implementasyondu. `security.py::check_environment_profile` (ve yalnız onun kullandığı `_read_env_file_assignments` private helper'ı) artık aynı redis.py deseninde gerçek gövdesiyle taşındı; `__init__.py`'deki orijinal `check_environment_profile` şimdi `checks/security.py`'ye deferred-import ile delege eden ince bir pass-through (döngüsel import'tan kaçınmak için `run_doctor_report()` içindeki gibi fonksiyon-lokal import kullanır). **Kritik detay:** `checks/security.py` `BASE_DIR`'i `from core.doctor import BASE_DIR` ile değer olarak değil, `import core.doctor as _doctor` + `_doctor.BASE_DIR` ile dinamik referans olarak kullanır — aksi halde `monkeypatch.setattr(doctor, "BASE_DIR", tmp_path)` kullanan testler (`test_environment_profile_passes_when_profile_file_exists_or_test_profile` ve `test_environment_profile_warns_when_development_database_is_not_isolated`) modülün kendi import-zamanı kopyasını değiştiremeyeceği için sessizce production `BASE_DIR`'i kullanmaya devam ederdi. `run_doctor_report()`'taki `environment_profile_check` deferred-import alias'ı kaldırıldı; artık `__init__.py`'nin kendi modül-global `check_environment_profile()` adını çağırıyor (monkeypatch-uyumlu, `test_run_doctor_report_writes_json_and_aggregates_warn`'ın `monkeypatch.setattr(doctor, "check_environment_profile", ...)` deseni korunuyor). `database.py`'nin 3 fonksiyonu (`check_database_env`, `check_database_connectivity`, `check_pgvector_ready`) de aynı desende gerçek gövdeleriyle taşındı; bunlarla birlikte yalnızca bu üç fonksiyonun call graph'ına özel 8 private helper (`_is_postgres_url`, `_redact_url`, `_redact_exception_text`, `_postgres_connectivity_failure_guidance`, `_source_message`, `_database_name`, `_validate_postgres_env_sync`, `_validate_database_url_pair_sync`) da `checks/database.py`'ye taşındı. **Kritik detay:** `_resolved_database_urls`, `_parse_url`, `_run_coro_sync`, `_probe_postgres_connectivity` ve `_dotenv_source_report` bilinçli olarak `__init__.py`'de bırakıldı — ilk dördü testlerde doğrudan `monkeypatch.setattr(doctor, "_run_coro_sync", ...)` gibi isimle hedeflenen monkeypatch seam'leri (ve `_resolved_database_urls`/`_parse_url` ayrıca `rag.py`'nin henüz taşınmamış fonksiyonlarıyla paylaşılıyor), `_dotenv_source_report` ise ilk denemede çok satırlı `monkeypatch.setattr(\n    doctor,\n    "_dotenv_source_report",\n    ...\n)` biçiminde kullanıldığı için tek satırlık grep taramasında gözden kaçıp yanlışlıkla taşınmıştı — testler bunu yakaladı, helper `__init__.py`'ye geri taşındı ve `checks/database.py` ona `_doctor._dotenv_source_report(...)` ile dinamik referans veriyor; `checks/database.py`'deki gerçek gövdeler bu 5 helper'ı hep `_doctor.X(...)` dinamik lookup ile çağırıyor, asla bare/local isimle değil. `gpu.py`'nin 3 fonksiyonu (`check_gpu_memory_config`, `check_docker_test_image`, `check_gpu`) da aynı desende gerçek gövdeleriyle taşındı (**düzeltme:** bu satırın önceki sürümü "gpu.py'nin 2'si" diyordu; dosyanın kendisi kontrol edildiğinde üç fonksiyonun de pass-through olduğu görüldü — orijinal incelemenin sayımı bir eksikti). `_run_command` (check_uv/check_migrations ile de paylaşılıyor, ayrıca `monkeypatch.setattr(doctor, "_run_command", ...)` seam'i) ve `_docker_image_exists_local` (doğrudan `monkeypatch.setattr(doctor, "_docker_image_exists_local", ...)` seam'i) aynı gerekçeyle `__init__.py`'de bırakıldı ve `checks/gpu.py` bunlara `_doctor.X(...)` ile erişiyor. `test_checks_modules.py`'deki artık yanlış yöne işaret eden `test_check_docker_test_image_delegates_to_doctor` kaldırıldı. `rag.py`'nin tamamı (`check_rag_index_ready`, `check_graphrag_entity_memory_ready`, `check_rag_readiness`) de aynı desende gerçek gövdeleriyle taşındı — bu, orijinal 5 dosyadaki 10 pass-through fonksiyonun (redis.py'nin kendi bağımsız implementasyonu hariç) tamamını kapsıyor. Yalnızca `check_rag_index_ready()`'in kullandığı `_ensure_rag_index_placeholder` helper'ı da `checks/rag.py`'ye taşındı. **Kritik detay:** `_rag_readiness_state` ve `_query_entity_graph_counts_from_store` bilinçli olarak `__init__.py`'de bırakıldı — ikisi de doğrudan `monkeypatch.setattr(doctor, "_rag_readiness_state", ...)` / `monkeypatch.setattr(doctor, "_query_entity_graph_counts_from_store", ...)` ile hedeflenen monkeypatch seam'leri; `_resolved_database_urls`/`_parse_url` (database.py dilimiyle paylaşılan, zaten `__init__.py`'de kalan seam'ler) ve `BASE_DIR` (security.py dilimindeki aynı nedenle) de `checks/rag.py`'den `_doctor.X` dinamik referansla erişiliyor. `_get_bool_env`/`_load_json_object` yalnızca `_rag_readiness_state()`'in kendi gövdesinde kullanıldığından, o `__init__.py`'de kaldığı için bu iki helper da taşınmadı. **Kritik detay #2 (yeni desen):** `check_rag_readiness()`, kendisiyle aynı dilimde taşınan `check_rag_index_ready()`/`check_graphrag_entity_memory_ready()`'i bare/local isimle değil `_doctor.check_rag_index_ready()` / `_doctor.check_graphrag_entity_memory_ready()` üzerinden çağırıyor — ikisi de doğrudan `monkeypatch.setattr(doctor, "check_rag_index_ready", ...)` gibi isimle hedeflenen seam'ler olduğundan (`tests/unit/core/test_doctor.py`'de birden fazla test bunları patch'leyip `doctor.check_rag_readiness()`'in patch'lenmiş sonucu kullandığını doğruluyor), aynı dosya içindeki iki fonksiyon arasında bile bare çağrı kullanmak bu testleri kırardı. `run_doctor_report()`'taki `rag_index_ready_check`/`graphrag_entity_memory_ready_check` deferred-import alias'ları kaldırıldı; artık bare `check_rag_index_ready()`/`check_graphrag_entity_memory_ready()` çağrılıyor. `test_checks_modules.py`'deki artık yanlış yöne işaret eden `test_check_rag_index_ready_delegates_to_doctor`, `test_check_graphrag_entity_memory_ready_delegates_to_doctor` ve `test_check_rag_readiness_delegates_to_doctor` kaldırıldı. `core/doctor/checks/*.py`'nin tamamı artık `redis.py::check_redis` ile aynı desende gerçek implementasyon barındırıyor; health check orchestration wrapperlarını `core/doctor/facade.py` sınırına daraltmak sıradaki adımdır. | `tests/unit/core/test_doctor.py`, `tests/unit/core/doctor/test_checks_modules.py`, CLI doctor smoke tests |
