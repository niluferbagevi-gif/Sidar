...
|---|---:|---|
| `web_server.py` | 2653 | Auth/JWT ile webhook HMAC/replay guard `web/security.py`, access-policy eşleme/serileştirme `web/middleware/access_policy.py`, rate-limit `web/middleware/ratelimit.py`, plugin policy `web/plugins/sandbox.py`, request modelleri `web/routes/request_models.py` ve route domainleri `web/routes/` altında ayrıldı; plugin sandbox policy üretimde fail-closed kalır, ana dosyada legacy monkeypatch/import wrapper azaltımı sürmeli. |
