...
| `install_sidar.sh` | 1903 | `scripts/install_modules/` yapısı var; help/UX yüzeyi ayrıldı, ana script bootstrap facade olarak inceltilmeli. |
| `config.py` | 1857 | Dotenv ve logging helperları ayrıldı; secret/runtime path boundary'leri daraltılmalı. |
...
| `install_sidar.sh` | Modüler installer modülleri olmasına rağmen bootstrap, UX, validation ve fallback yüzeyi ana scriptte yoğun kalıyor. | `scripts/install_modules/bootstrap.sh`, `scripts/install_modules/utils/ux.sh`, `scripts/install_modules/validation.sh`, `scripts/install_modules/offline_bundle.sh` | CLI help/UX çıktısı `scripts/install_modules/utils/ux.sh` içine çıkarıldı; sıradaki düşük riskli adım ana scripti yalnız argument parsing + phase dispatcher facade'ına indirirken bootstrap sözleşmesini korumak. | installer smoke, `tests/unit/scripts/*`, shellcheck |
...
| `config.py` | Env yükleme, secret hardening, GPU detect orkestrasyonu, runtime path inference ve logging bootstrap tek facade içinde yoğunlaşıyor. | `core/config_dotenv.py`, `core/config_dirs.py`, `core/config_gpu_detect.py`, `core/config_logging_setup.py`, `core/config_runtime_env.py`, `core/config_secrets.py` | Dotenv parse/load/reload tanılama helperları `core/config_dotenv.py`, logging bootstrap facade helperları `core/config_logging_setup.py` içine çıkarıldı; sıradaki düşük riskli adım secret hardening ve runtime path inference wrapperlarını facade dışına taşımak. | `tests/unit/root/test_config.py`, boot diagnostics tests |
