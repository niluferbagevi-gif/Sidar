...
| `managers/code_manager.py` | 1615 | Docker/patch/security/shell helperları ayrıldı; sandbox lifecycle adapter'ı sonraki güvenli adım. |
...
| `managers/code_manager.py` | Shell execution, Docker sandbox, patching, LSP adapter ve güvenlik kontrolleri tek sınıfta. | `managers/code/runner.py`, `managers/code/docker.py`, `managers/code/patcher.py`, `managers/code/lsp.py`, `managers/code/security_adapter.py` | `docker.py`, `patcher.py`, `security_adapter.py` ve `runner.py` eklendi; `CodeManager` Docker CLI/sandbox limit, patch, IO güvenlik ve local shell yürütme kararlarını bu helperlara delege ediyor. Sıradaki düşük riskli adım Docker SDK lifecycle ve shell sandbox adapter sınıflarını ayırmak. | `tests/unit/managers/test_code_manager.py`, sandbox contract tests |
