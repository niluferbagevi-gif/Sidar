...
| `core/llm_client.py` | Tüm LLM provider adapter'ları, routing, semantic cache, DLP, retry, tracing ve Ollama stream parsing tek dosyada. | `core/llm/providers/ollama.py`, `core/llm/providers/openai.py`, `core/llm/providers/gemini.py`, `core/llm/providers/anthropic.py`, `core/llm/providers/litellm.py`, `core/llm/router.py`, `core/llm/streaming.py`, `core/llm/cache.py`, `core/llm/facade.py` | Provider sınıflarını public `LLMClient` facade'ını bozmadan alt modüllere taşıyacak import boundary ve contract testlerini eklemek. | `tests/unit/core/test_llm_client.py`, provider contract tests |
