...
| `config.py` | 1776 | Dotenv, logging, secret hardening, runtime path inference, rate-limit/Redis, event bus/Kafka, RAG store, Docker sandbox, pazarlama/sosyal entegrasyon, GitHub/Hugging Face, web arama, cost-aware routing, LoRA/QLoRA ve sürekli öğrenme (continuous learning) loaderları ayrıldı; facade hâlâ çok sayıda class attribute compatibility yüzeyi taşıyor. |
