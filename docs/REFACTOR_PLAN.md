...
| `config.py` | 1785 | Dotenv, logging, secret hardening, runtime path inference, rate-limit/Redis, event bus/Kafka, RAG store, Docker sandbox, pazarlama/sosyal entegrasyon, GitHub/Hugging Face, web arama, cost-aware routing, LoRA/QLoRA, sürekli öğrenme (continuous learning), multimodal vision/ses, tarayıcı otomasyonu ve LSP entegrasyonu loaderları ayrıldı; facade hâlâ çok sayıda class attribute compatibility yüzeyi taşıyor. |
