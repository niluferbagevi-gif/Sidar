...
|---|---|---|---|---|
| `web_server.py` | Endpoint, WebSocket, middleware, auth, frontend fallback ve bootstrap tek dosyada yoğunlaşıyor. | `web/routes/ws_chat.py`, `web/routes/ws_voice.py`, `web/routes/collaboration.py`, `web/routes/webhooks.py`, `web/routes/operations.py`, `web/security.py`, `web/app_factory.py` | App factory çıkarımı yapıldı; sıradaki adım `/ws/chat` için router factory + legacy export shim. | `tests/unit/root/test_web_server.py`, `tests/unit/web/*`, smoke boot tests |
