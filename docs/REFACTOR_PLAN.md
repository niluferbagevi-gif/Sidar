...
| `config.py` | 1781 | Dotenv, logging, secret hardening, runtime path inference, rate-limit/Redis, event bus/Kafka, RAG store, Docker sandbox, pazarlama/sosyal entegrasyon, GitHub/Hugging Face, web arama, cost-aware routing, LoRA/QLoRA, sürekli öğrenme (continuous learning), multimodal vision/ses, tarayıcı otomasyonu, LSP entegrasyonu ve otonomi servisi webhook loaderları ayrıldı; facade hâlâ çok sayıda class attribute compatibility yüzeyi taşıyor. |
