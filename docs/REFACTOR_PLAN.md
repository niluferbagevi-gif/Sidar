...
| `config.py` | 1787 | Dotenv, logging, secret hardening, runtime path inference, rate-limit/Redis, event bus/Kafka, RAG store, Docker sandbox, pazarlama/sosyal entegrasyon, GitHub/Hugging Face, web arama, cost-aware routing, LoRA/QLoRA, sürekli öğrenme (continuous learning), multimodal vision/ses, tarayıcı otomasyonu, LSP entegrasyonu, otonomi servisi webhook, RAG LLM entity extraction ve web arayüzü bind loaderları ayrıldı; `TEXT_MODEL` mevcut typed LLM settings domain'ine alındı; facade hâlâ çok sayıda class attribute compatibility yüzeyi taşıyor. |
