...
| `core/db.py` | Record modelleri, auth, migration, repository ve operational persistence aynı dosyada. | `core/db/auth.py`, `core/db/audit.py`, `core/db/coverage.py`, `core/db/marketing.py`, `core/db/session.py`, ortak `core/db/models.py` | `audit.py`, `sessions.py` ve `metrics.py` akışları çıkarıldı; `audit_log.py` legacy compatibility wrapper olarak bırakıldı. Sıradaki düşük riskli adım read-only serializer/query helper'larını domain modüllerine taşıyıp `core.db` üzerinden re-export etmek. | `tests/unit/core/test_db.py`, migration integration tests |
