...
> **Son Güncelleme:** 2026-03-18 (v3.0.26 — O-8, D-7, D-8 ve D-9 numaralı bulgular kapatılarak teknik borç azaltıldı. Güncel açık bulgular: 1 Yüksek (Y-6), 1 Orta (O-7) ve 5 Düşük (D-10..D-14). D-10 ve D-11 için çözüm girişimi eksik kaldığından rapor açık statüde tutuldu. Güvenlik puanı 9.2→9.4 olarak revize edildi.)
...
- **[Çözüldü — v3.0.26] O-8 / Slack init blokajı:** `SlackManager` içindeki senkron `auth_test()` çağrısı kaldırıldı; doğrulama asenkron `initialize()` akışına ve `asyncio.to_thread(...)` sarmalamasına taşındı.
- **[Çözüldü — v3.0.26] D-7 / Prometheus gauge tekrar kaydı:** `core/judge.py` içinde gauge önbelleği eklenerek aynı metrik adının tek sefer kaydedilmesi sağlandı.
- **[Çözüldü — v3.0.26] D-8 / Entity memory ölü kodu:** `core/entity_memory.py` içindeki no-op atama kaldırıldı.
- **[Çözüldü — v3.0.26] D-9 / Cache metrics public API:** `core/cache_metrics.py` içine thread-safe `record_*` metotları eklenerek private singleton bağımlılığı kaldırıldı.
...
### 11.1.A Aktif Açık Bulgular (Güncel)

| ID | Durum | Not |
|---|---|---|
| Y-6 | 🔴 AÇIK | `record_routing_cost()` çağrısı hâlâ eksik; günlük bütçe izleyici işlevsiz. |
| O-7 | 🔴 AÇIK | Vision/EntityMemory/FeedbackStore/Slack/Jira/Teams modülleri için `web_server.py` HTTP endpoint entegrasyonu yok. |
| D-10 | 🟡 AÇIK | `core/judge.py` içinde `Config()` örneği hâlâ `_call_llm()` içinde oluşturuluyor. |
| D-11 | 🟡 AÇIK | `core/vision.py` `load_image_as_base64()` içinde senkron `read_bytes()` kullanıyor; `asyncio.to_thread` uygulanmadı. |
| D-12 | 🟡 AÇIK | `core/active_learning.py` içinde f-string SQL deseni sürüyor. |
| D-13 | 🟡 AÇIK | `core/hitl.py` içinde `asyncio.Lock()` event loop dışında örnekleniyor. |
| D-14 | 🟡 AÇIK | `web_server.py` özel `_notify()` fonksiyonunu modül dışından import ediyor. |
...
- **Final Doğrulama Tarihi:** 2026-03-18 (Güncel — v3.0.14)
- **Durum:** Kurumsal özellikler (Auth, Multi-User, Sandbox, Observability, Plugin Marketplace, Tenant RBAC) kod ve test düzeyinde doğrulanmıştır. v3.0.14'te tüm kaynak dosyalar yeniden satır satır ölçülmüş; `agent/registry.py`, `agent/swarm.py` ve `plugins/upload_agent.py` dosya ağacına ve bağımlılık haritasına işlenmiş; test ve satır sayısı istatistikleri gerçek değerlere göre düzeltilmiştir. Aktif açık bulgular Y-6, O-7 ve D-10..D-14 ile sınırlıdır; O-8 ile D-7..D-9 v3.0.26 revizyonunda kapatılmıştır.
...
| **v3.0.26** | **2026-03-18** | **Rapor revizyonu: O-8 ile D-7, D-8 ve D-9 kapatıldı. Aktif açık bulgular Y-6, O-7 ve D-10..D-14 olarak daraltıldı. D-10 (`core/judge.py` içinde `Config()` hâlâ `_call_llm()` içinde) ve D-11 (`core/vision.py` içinde senkron `read_bytes()`) için çözüm girişimlerinin eksik kaldığı özellikle not edildi. Güvenlik puanı 9.4 olarak güncellendi.** |
