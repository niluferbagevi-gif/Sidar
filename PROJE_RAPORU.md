...
| Olası Neden | Nereden Anlaşılır | Çözüm |
|-------------|-------------------|-------|
| `chromadb` kurulmamış | `rag.py:128` import kontrolü (`self._check_import("chromadb")`) | `pip install chromadb` |
| `sentence-transformers` yok | `rag.py:46` GPU embedding importu (`SentenceTransformerEmbeddingFunction`) | `pip install sentence-transformers` |
...
| ChromaDB versiyon uyumsuzluğu | `rag.py:199` Chroma başlatma hata logu (`ChromaDB başlatma hatası`) | `pip install chromadb --upgrade` |
| `chunk_size < chunk_overlap` | `rag.py:276` (`step = max(1, size - overlap)`) ve `rag.py:307` (`overlap_len = min(...)`) | `RAG_CHUNK_OVERLAP < RAG_CHUNK_SIZE` olduğundan emin ol |

> Not: Bu bölümdeki satır numaraları, güncel `core/rag.py` dosyasına göre revize edilmiştir.
...
| Olası Neden | Nereden Anlaşılır | Çözüm |
|-------------|-------------------|-------|
| Geçersiz Fernet anahtarı | `config.py:457-471` (`validate_critical_settings`) | `python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"` ile yeni anahtar üret |
| `cryptography` kurulmamış | `config.py:472-478` ImportError logu | `pip install cryptography` |
| SQLite DB dosyası/klasörü yazma izni yok | `core/memory.py:39-44` ve `core/db.py` SQLite yazma hataları | `data/sidar_memory.db` (veya `DATABASE_URL` ile verilen SQLite yolu) için `chmod/chown` izinlerini düzelt |
| Veritabanı bağlantı/erişim hatası | `core/memory.py` DB çağrılarında (`self.db.*`) veya PostgreSQL pool oluşturma loglarında hata | `DATABASE_URL` değerini ve veritabanı servisinin çalıştığını doğrula |

> Not: v3.0 ile bellek katmanı DB tabanlıdır; `core/memory.py:_cleanup_broken_files()` artık legacy dosya karantinası için no-op'tur.
