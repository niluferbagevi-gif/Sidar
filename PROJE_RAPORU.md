...
**Durum:** Arşivlendi (çözülen bulgular CHANGELOG'a taşındı).

Bu bölümde listelenen **K-1..K-2, Y-1..Y-5, O-1..O-7, D-1..D-6** bulgularının tamamı v3.0.5 denetimi kapsamında kapatılmıştır (yanlış pozitif olanlar ayrıca teyit edilmiştir).

Raporun okunabilirliğini korumak ve tekrar eden içerik şişmesini önlemek için, 11.3 altındaki ayrıntılı bulgu/çözüm satırları bu dosyadan kaldırılmış ve sürüm geçmişine taşınmıştır:

- Ayrıntılı kayıtlar: [CHANGELOG.md → v3.0.5](CHANGELOG.md#v305---2026-03-16)
- Kapsam özeti: v3.0.4 bulgularının doğrulanması/kapanışı (K/Y/O/D) + yanlış pozitif teyitleri

> Not: PROJE_RAPORU.md içinde yalnızca güncel/açık bulguların özeti tutulur; kapatılan bulguların detayları CHANGELOG'da izlenir.
