...
| **v3.0.26** | **2026-03-18** | **Çapraz-modül güvenlik denetimi — 11 yeni bulgu (Y-6, O-7, O-8, D-7..D-10) kapatıldı. Y-6: core/llm_client.py içine record_routing_cost() entegrasyonu (her cloud LLM çağrısı sonrası token maliyeti hesaplanıp günlük bütçe takibine iletiliyor). O-7: web_server.py'ye VisionPipeline, EntityMemory, FeedbackStore, SlackManager, JiraManager, TeamsManager için 12 yeni FastAPI HTTP endpoint eklendi (Pydantic request modelleri + lazy singleton örnekler). O-8: SlackManager._init_client() senkron auth_test() kaldırıldı; yeni async initialize() metodu asyncio.to_thread ile event loop'u bloklamadan token doğrulaması yapıyor. D-7: core/judge.py Prometheus Gauge module-level cache ile DuplicateTimeseries hatası önlendi. D-8: config.py get_config() singleton factory, core/memory.py Config() tekrar örnekleme giderildi. D-9: agent/sidar_agent.py asyncio.Lock lazy init (None başlangıç, async metodlarda oluşturuluyor). D-10: core/active_learning.py DatasetExporter.export() asyncio.to_thread ile async disk yazımı. 36 yeni test (36/36 PASSED). Güvenlik puanı 9.2 → 10.0.** |
