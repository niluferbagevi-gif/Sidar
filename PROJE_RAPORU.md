...
- [14. Gelecek Vizyonu ve Geliştirme Yol Haritası (Faz 6 / v5.x)](#14-gelecek-vizyonu-ve-geliştirme-yol-haritası-faz-6--v5x)
  - [14.1 GraphRAG ve Bilgi Grafikleri (Knowledge Graphs)](#141-graphrag-ve-bilgi-grafikleri-knowledge-graphs)
  - [14.2 Dağıtık Sürü Orkestrasyonu (Distributed Swarm via Message Brokers)](#142-dağıtık-sürü-orkestrasyonu-distributed-swarm-via-message-brokers)
  - [14.3 Sürekli Öğrenme ve RLHF/DPO Boru Hattı](#143-sürekli-öğrenme-ve-rlhfdpo-boru-hattı)
  - [14.4 Gerçek Zamanlı Çoklu Modalite (Realtime WebRTC)](#144-gerçek-zamanlı-çoklu-modalite-realtime-webrtc)
  - [14.5 Faz 6 Yol Haritası Özeti](#145-faz-6-yol-haritası-özeti)
  - [14.6 Tam Otonom Test ve Coverage Yönetimi](#146-tam-otonom-test-ve-coverage-yönetimi)
  - [14.7 İş Operasyonları ve Dijital Pazarlama (Poyraz Ajanı)](#147-iş-operasyonları-ve-dijital-pazarlama-poyraz-ajanı)
  - [14.8 Auto-Healing ve İleri Düzey Görsel Otonomi](#148-auto-healing-ve-i̇leri-düzey-görsel-otonomi)
...
- **Multi-Agent + P2P Delegasyon:** Supervisor orkestrasyonu ile görevleri uzman rollere (Coder, Researcher, Reviewer) dağıtır; `agent/core/contracts.py` ile ajanlar arası P2P görev sözleşmesi desteklenir. *(Gelecek vizyonunda: Kapsam/Test Ajanı ve Dijital Pazarlama/Poyraz Ajanı.)*
...
│   ├── multimodal.py          # Video frame + STT tabanlı medya bağlamı oluşturma (YENİ — v5.0-alpha)
│   └── voice.py               # TTS adaptörleri ve WebSocket ses segmentasyonu (YENİ — v5.0-alpha)
│
...
│   ├── browser_manager.py     # Playwright/Selenium + HITL korumalı tarayıcı otomasyonu (YENİ — v5.0-alpha)
...
### 14.6 Tam Otonom Test ve Coverage Yönetimi

- `%99.9` coverage hard gate kültürünü insan müdahalesi olmadan sürdüren özel bir uzman ajan rolü planlanmaktadır.
- Bu rol, coverage raporlarını okuyup eksik branch/satırları hedefleyen testleri kendisi yazacak ve reviewer kalite kapısıyla birlikte kapalı döngü çalışacaktır.
- Beklenen kazanım: büyük refactor'larda kapsam düşüşünün proaktif tespiti ve edge-case testlerinin sürekli tamamlanması.

### 14.7 İş Operasyonları ve Dijital Pazarlama (Poyraz Ajanı)

- Sidar'ın dışa dönük işletme ajanı olarak konumlanan Poyraz Ajanı; sosyal medya içerikleri, müşteri iletişimi ve kampanya operasyonlarını yönetecek şekilde tasarlanmaktadır.
- WhatsApp, Instagram ve benzeri kanallarla entegrasyon; sadece bildirim değil, içerik üretimi, yayın planlama ve dönüşüm analizi katmanlarını da kapsayacaktır.
- Beklenen kazanım: yazılım yaşam döngüsünü yöneten platformun, müşteri ve pazar teması olan otonom iş ekosistemine dönüşmesi.

### 14.8 Auto-Healing ve İleri Düzey Görsel Otonomi

- Docker/web sunucu loglarından kendi kendine onarım döngüsü başlatan auto-healing hattı, GraphRAG destekli kök neden analizi ile birleşecektir.
- Tarayıcı otomasyonunda ise DOM seçicilerini tamamlayan Set-of-Mark tabanlı görsel karar mekanizmaları ve YouTube gibi platformlardan doğrudan analiz yeteneği hedeflenmektedir.
- Beklenen kazanım: daha düşük operasyonel kesinti, daha güçlü görsel navigasyon ve gerçek dünya platformlarıyla daha doğrudan etkileşim.
...
| **Dijital Pazarlama Otonomisi** | `poyraz_agent.py`, `managers/social_manager.py` (Planlanan) | Tasarım Aşamasında |
