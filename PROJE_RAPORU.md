...
├── <a href="docs/module-notes/docker-compose.yml.md">docker-compose.yml</a>         # 5 servis + Prometheus &amp; Grafana entegrasyonu
...
| 3 | Sağlayıcılar arası tool-calling şema farkları | `core/llm_client.py`, `agent/tooling.py` | OpenAI/Anthropic/Gemini format farkları if/else yüzeyini büyütüyor; soyutlama sızıntısı riski | Orta | ✅ **KAPALI** |
|   | ↳ Not | `agent/tooling.py`, `agent/sidar_agent.py`, `core/llm_client.py` | Legacy `|||` argüman ayrıştırması kaldırıldı, şemalı araçlar için JSON object zorunlu hale getirildi; sağlayıcı-özel JSON mode dönüşümü `build_provider_json_mode_config(...)` ile tek adaptör altında toplandı | - | Bilgi |
