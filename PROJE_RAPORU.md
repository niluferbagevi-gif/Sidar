...
**Arama Modları (v2.10.0 — §14.3.1 RRF + §14.3.2 FTS5 + §14.3.3 İzolasyon):**

| Mod | Motor | Açıklama |
|-----|-------|----------|
| `auto` | **RRF (ChromaDB + BM25 FTS5)** → ChromaDB → BM25 → Keyword | Her iki motor mevcutsa RRF öncelikli |
| `vector` | ChromaDB (cosine similarity + `session_id` where filtresi) | Anlamsal arama |
| `bm25` | SQLite FTS5 (`session_id` SQL filtresi, disk tabanlı) | TF-IDF benzeri kelime sıklığı — sıfır RAM |
...
- `_fetch_bm25()`: SQLite FTS5 MATCH + `session_id = ?` filtresi — oturum izolasyonu SQL düzeyinde
...
| `sqlite3` | ✓ (built-in) | BM25 RAG (FTS5) — v2.10.0'dan itibaren `rank_bm25` gerekmez |
...
**Güncel durum:** ✅ **v2.10.0'da tamamlandı.**

`rank_bm25` (in-memory) tamamen kaldırılarak Python standart kütüphanesinin SQLite FTS5 motoru ile değiştirildi:

| Değişiklik | Eski (`rank_bm25`) | Yeni (SQLite FTS5) |
|---|---|---|
| Bellek kullanımı | Tüm corpus RAM'de (`_bm25_corpus_tokens`) | Disk tabanlı (`bm25_fts.db`) — RAM tüketimi sıfır |
| Başlatma | `BM25Okapi(tokens)` her güncellemede yeniden kurulur | `CREATE VIRTUAL TABLE IF NOT EXISTS bm25_index USING fts5(...)` — tek seferlik |
| Tokenizer | `str.lower().split()` | `unicode61 remove_diacritics 1` — Türkçe karakter desteği |
| Sorgulama | Python döngüsü + `get_scores()` | `MATCH ? AND session_id = ?` — milisaniye düzeyinde |
| Bağımlılık | `rank_bm25` pip paketi | Python built-in `sqlite3` — sıfır bağımlılık |
| Geriye dönük uyum | — | İlk başlatmada mevcut belgeler otomatik migrate edilir |

**Uygulanan değişiklikler (`core/rag.py`):**
- `_init_fts()`: SQLite bağlantısı açar, FTS5 sanal tablosunu oluşturur, mevcut `_index` belgelerini migrate eder
- `_update_bm25_cache_on_add()`: Yeni belgeyi FTS5 tablosuna ekler (kilit reentrancy deadlock'u önlendi — `_write_lock` dışarıda tutuldu)
- `_update_bm25_cache_on_delete()`: Belgeyi FTS5 tablosundan siler
- `_fetch_bm25()`: `bm25()` fonksiyonu ile skorlama; negatif değerler `abs()` ile pozitife çevrilir; `session_id` filtresi SQL düzeyinde uygulanır
- Silinen metodlar: `_invalidate_bm25_cache`, `_rebuild_bm25_from_cache`, `prebuild_bm25_index`, `_ensure_bm25_index`
...
> **Durum Notu:** v2.7.0–v2.10.0 arasında tüm yüksek/orta öncelikli maddeler tamamlandı. Kalan açık maddeler düşük etki / opsiyonel kategorisindedir.
...
| — | BM25 corpus ölçeklenebilirliği → SQLite FTS5 (§14.3.2) | Orta | Yüksek | ✅ **v2.10.0** |
...
| BM25 arama (SQLite FTS5) | `sqlite3` (built-in) | — |
...
| **Tam CLI** (+ BM25 + GitHub + web arama) | yukarıdaki + `PyGithub`, `duckduckgo-search` (`rank_bm25` v2.10.0'dan itibaren gerekmez) |
