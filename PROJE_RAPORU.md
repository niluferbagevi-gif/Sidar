...
- [6. Test Kapsamı ve Kalite Güvencesi (QA)](#6-test-kapsamı-ve-kalite-güvencesi-qa)
  - [6.1 Birim (Unit) ve Uç Durum (Edge Case) Testleri](#61-birim-unit-ve-uç-durum-edge-case-testleri)
  - [6.2 Kalite Kapıları (Quality Gates) ve Konfigürasyon](#62-kalite-kapıları-quality-gates-ve-konfigürasyon)
  - [6.3 Sürekli Entegrasyon (CI/CD) Boru Hattı](#63-sürekli-entegrasyon-cicd-boru-hattı)
...
## 6. Test Kapsamı ve Kalite Güvencesi (QA)

[⬆ İçindekilere Dön](#içindekiler)

Sidar projesi, kurumsal güvenilirlik ve sıfır hata (zero-defect) vizyonuyla hareket ederek v3.0 sürümünde test altyapısını baştan aşağı yenilemiştir. Projede 69'dan fazla test modülü ve ~20.000+ satırlık test kodu ile "Maximum Kapsama (Max Coverage)" hedeflenmektedir.

### 6.1 Birim (Unit) ve Uç Durum (Edge Case) Testleri
- **Kapsamlı Rol Testleri:** Ajanların kararları ve yetenekleri (`test_coder_agent.py`, `test_reviewer_agent.py`), hibrit RAG motoru (`test_rag_ultimate_edge_cases.py`) ve LLM istemcileri detaylı olarak test edilmektedir.
- **Olağandışı Durum Simülasyonları:** `test_missing_edge_case_coverage_final.py`, `test_ultimate_coverage.py`, `test_quick_100.py` ve `test_coverage_final_push.py` gibi özel modüller kullanılarak ağ kopmaları, token limit aşımları ve hatalı LLM yanıtları gibi senaryolarda sistemin çökmeden (Fail-Safe) nasıl davranacağı güvence altına alınmıştır.
- **Mocking Altyapısı:** Gerçek API maliyetlerini ve ağ gecikmelerini önlemek amacıyla, LLM istekleri ile dosya/sandbox operasyonlarının önemli bir kısmı mock yapılarıyla izole edilerek test edilmektedir.

### 6.2 Kalite Kapıları (Quality Gates) ve Konfigürasyon
- **Standardizasyon:** Test süreçleri `pytest.ini` ve kapsama analizleri `.coveragerc` üzerinden katı kurallara bağlanmıştır.
- **Denetim Betikleri:** `run_tests.sh`, `scripts/audit_metrics.sh` ve `scripts/check_empty_test_artifacts.sh` betikleri sayesinde, test/coverage eşiklerinin altına düşen veya bozuk test artifact'i üreten değişikliklerin sürece dahil edilmesi engellenir.

### 6.3 Sürekli Entegrasyon (CI/CD) Boru Hattı

Proje, GitHub Actions üzerinde tam otomatik bir Sürekli Entegrasyon (CI) hattına sahiptir:
- **Kod Kalitesi:** Her kod gönderiminde (Push/PR) Ruff ile stil (Linting) ve Mypy ile statik tip (Type-Checking) doğrulaması zorunlu tutulur.
- **İzole Test Ortamı:** `.github/workflows/ci.yml` iş akışı, Linux ortamında izole Python kurulumuyla test suitini çalıştırır ve kalite kapılarını uygular.
- **Veritabanı Güvenlik Ağı:** `.github/workflows/migration-cutover-checks.yml` ile PostgreSQL/SQLite şema geçişlerinin (Alembic) ve data migration senaryolarının güvenli olup olmadığı üretim (production) öncesi otomatik denetlenir.
