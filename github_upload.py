...
def _git_lines_z(args: list[str]) -> list[str]:
    ...
    try:
        ...
        err_msg = (e.stderr or b"").decode("utf-8", errors="replace").strip()
        if e.stdout:
...
def _run_git_in_batches(base_cmd: list[str], paths: list[str], batch_size: int = GIT_BATCH_SIZE) -> None:
    """Uzun argüman listesi hatalarını önlemek için komutu parçalı çalıştırır."""
...
def stage_plan(plan: SyncPlan, cli: CliArgs) -> None:
    ...
    if plan.add_or_update:
        _run_git_in_batches(["git", "add", "--"], plan.add_or_update)
