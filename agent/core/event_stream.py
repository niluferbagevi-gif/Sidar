...
class AgentEventBus:
    def __init__(self) -> None:
        ...
        self._dlq_persist_flush_interval = max(
            0.05, float(getattr(cfg, "SIDAR_EVENT_BUS_DLQ_PERSIST_FLUSH_INTERVAL", 1.0) or 1.0)
        )
        ...
        self._redis_max_connections = max(1, int(getattr(cfg, "REDIS_MAX_CONNECTIONS", 50) or 50))
        self._redis_connect_timeout = float(getattr(cfg, "REDIS_CONNECT_TIMEOUT", 0.5) or 0.5)
        self._redis_socket_timeout = float(getattr(cfg, "REDIS_SOCKET_TIMEOUT", 0.5) or 0.5)
        ...
        self._kafka_topic = str(getattr(cfg, "SIDAR_EVENT_BUS_KAFKA_TOPIC", "sidar.agent_events"))
