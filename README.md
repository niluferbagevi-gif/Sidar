...
- `postCreateCommand`, `.devcontainer/setup-codespaces.sh post-create` ile `uv venv .venv` ve `uv sync --frozen --all-extras` çalıştırarak geliştirici bağımlılıkları ve opsiyonel entegrasyonları hazırlar. `updateContentCommand` aynı senkronizasyon yolunu kullanır. Betik, taşınmış/yeniden oluşturulmuş workspace'lerde görülen container dışı `.venv/bin/python` symlink'lerini, eksik `pyvenv.cfg home` yollarını ve yanlış Python minor sürümünü uv sync öncesinde temizleyip sanal ortamı deterministik yeniden kurar. Başarılı sync sonrası `.venv/.sidar-devcontainer-sync.stamp` yazıldığı için aynı açılışta dependency girdileri değişmediyse tekrar eden `uv sync` atlanır; zorunlu yenileme için `SIDAR_DEVCONTAINER_FORCE_UV_SYNC=1` kullanılabilir.
