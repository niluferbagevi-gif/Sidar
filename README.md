...
- Ollama CLI varsayılan olarak kurulmaya çalışılır, servis `postStartCommand` ile arka planda başlatılır ve standart coding modeli `qwen2.5-coder:7b` olarak tanımlanır. Setup betiği proje kökündeki `.sidar-ollama.sha256` parmak iziyle Ollama binary yolu/sürümü, binary SHA256 değeri, normalize edilmiş `OLLAMA_REQUIRED_MODELS` listesi ve bu modellerin yerel envanterini izler; parmak izi güncelse binary/model senkronizasyonunu atlayarak gereksiz tekrar indirmeleri önler. Büyük model indirmesini Codespaces açılışında zorunlu kılmak istemiyorsanız varsayılan `SIDAR_CODESPACES_PULL_OLLAMA_MODELS=0` kalır; önceden indirme için Codespaces secret/env olarak `SIDAR_CODESPACES_PULL_OLLAMA_MODELS=1`, parmak izi yenilemeyi zorlamak için `SIDAR_FORCE_OLLAMA_SETUP=1` verebilirsiniz.
