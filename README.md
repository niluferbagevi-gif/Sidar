...
- `postCreateCommand`, `.devcontainer/setup-codespaces.sh post-create` ile `uv venv .venv` ve `uv sync --frozen --all-extras` çalıştırarak geliştirici bağımlılıkları ve opsiyonel entegrasyonları hazırlar. Dev Container, `.venv` yolunu `${localWorkspaceFolderBasename}-sidar-venv` adlı Docker volume ile izole eder; böylece WSL/host tarafında oluşmuş `.venv/bin/python -> /home/...` benzeri mutlak symlink'ler container açılışını tekrar tekrar kirletmez. `updateContentCommand` aynı senkronizasyon yolunu kullanır; ancak `.venv/.sidar-uv-sync.sha256` parmak izi `pyproject.toml`, `uv.lock`, `.python-version`, Python minor sürümü ve standart sync komutuyla eşleşiyorsa tekrar eden `uv sync` adımını atlar. Zorunlu yeniden senkronizasyon için `SIDAR_FORCE_UV_SYNC=1 bash .devcontainer/setup-codespaces.sh sync` kullanılabilir. Betik, taşınmış/yeniden oluşturulmuş workspace'lerde görülen container dışı `.venv/bin/python` symlink'lerini, eksik `pyvenv.cfg home` yollarını, volume sahipliği sorunlarını ve yanlış Python minor sürümünü uv sync öncesinde temizleyip sanal ortamı deterministik yeniden kurar.
