...
>
> **GPU Compose Ollama adresi notu:** `docker-compose.yml`'daki `sidar-ai`/`sidar-web` (CPU profili) `ollama` servisine, `sidar-gpu`/`sidar-web-gpu` (GPU profili) ise `ollama-gpu` servisine `depends_on` ile bağlıdır; her iki servisin `OLLAMA_URL` varsayılanı da kendi profiline uygun compose servis adını (`http://ollama:11434/api` / `http://ollama-gpu:11434/api`) hedefler ve bu, `${SIDAR_CONTAINER_OLLAMA_URL}` adlı ayrı bir override değişkenine bağlıdır — `.env`'deki host-odaklı `OLLAMA_URL` (`http://localhost:11434/api`, yalnızca `python main.py`/local runtime için doğru) bu container varsayılanını **gölgelemez**. Container'ların farklı/harici bir Ollama uç noktasına bağlanması gerekiyorsa `.env`'de `SIDAR_CONTAINER_OLLAMA_URL` açıkça tanımlanmalıdır (bkz. `.env.advanced.example`).
