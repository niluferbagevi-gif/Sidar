...
- Pyright LSP binary gereksinimi `pyright-langserver --stdio` olarak doğrulanır; `pyproject.toml` içindeki `pyright>=1.1.409,<2.0.0` kaydı `uv sync --frozen --all-extras` ile proje ortamına kurulur. Docker test imajı build sırasında `pyright` ve `pyright-langserver` binary'lerini doğrular. `CodeManager` PATH dışında aktif/proje `.venv` ve `~/.local/bin` adaylarını da tarar; binary yine bulunamazsa Python LSP için önce repo kilidine bağlı `uv run --frozen pyright-langserver --stdio`, uv sync yapılmamışsa `uvx --from pyright pyright-langserver --stdio` fallback'i kullanılır. `install_sidar.sh sync-deps` ve Dev Container post-create/sync akışı binary yine bulunamazsa `uv tool install pyright` fallback'ini çalıştırır.
...
>
> **GPU Compose Ollama adresi notu:** `docker-compose.yml`'daki `sidar-agent`/`sidar-web` (CPU profili) `ollama` servisine, `sidar-agent-gpu`/`sidar-web-gpu` (GPU profili) ise `ollama-gpu` servisine `depends_on` ile bağlıdır; her iki servisin `OLLAMA_URL` varsayılanı da kendi profiline uygun compose servis adını (`http://ollama:11434/api` / `http://ollama-gpu:11434/api`) hedefler ve bu, `${SIDAR_CONTAINER_OLLAMA_URL}` adlı ayrı bir override değişkenine bağlıdır — `.env`'deki host-odaklı `OLLAMA_URL` (`http://localhost:11434/api`, yalnızca `python main.py`/local runtime için doğru) bu container varsayılanını **gölgelemez**. Container'ların farklı/harici bir Ollama uç noktasına bağlanması gerekiyorsa `.env`'de `SIDAR_CONTAINER_OLLAMA_URL` açıkça tanımlanmalıdır (bkz. `.env.advanced.example`).
...
- `postCreateCommand`, `.devcontainer/setup-codespaces.sh post-create` ile `uv venv .venv-container` ve `uv sync --frozen --all-extras` çalıştırarak geliştirici bağımlılıkları ve opsiyonel entegrasyonları container'a özel sanal ortamda hazırlar. `updateContentCommand` aynı senkronizasyon yolunu kullanır; ancak `.venv-container/.sidar-uv-sync.sha256` parmak izi `pyproject.toml`, `uv.lock`, `.python-version`, Python minor sürümü ve standart sync komutuyla eşleşiyorsa tekrar eden `uv sync` adımını atlar. Zorunlu yeniden senkronizasyon için `SIDAR_FORCE_UV_SYNC=1 bash .devcontainer/setup-codespaces.sh sync` kullanılabilir. Host/WSL kurulumlarında kullanılan `.venv` korunur; Dev Container ise `UV_PROJECT_ENVIRONMENT=.venv-container` ve VS Code yorumlayıcısı `${containerWorkspaceFolder}/.venv-container/bin/python` ile izole edilir. Betik, taşınmış/yeniden oluşturulmuş workspace'lerde hedef container ortamındaki kırık `bin/python` symlink'lerini, eksik `pyvenv.cfg home` yollarını ve yanlış Python minor sürümünü uv sync öncesinde temizleyip sanal ortamı deterministik yeniden kurar.
- Dev Container, host/WSL tarafındaki uv paket önbelleğini `${localEnv:HOME}/.cache/uv` kaynağından `/home/vscode/.cache/uv` hedefine bind mount eder ve `UV_CACHE_DIR=/home/vscode/.cache/uv` kullanır. `initializeCommand` bu host dizinini önceden oluşturur; böylece `.venv-container` hâlâ container'a özel kalırken konteyner silinip yeniden kurulduğunda PyTorch/SciPy gibi büyük paket arşivleri mümkün olduğunca tekrar indirilmez.
...
Playwright E2E akışı gerçek backend ile çakışmamak için işletim sisteminden dinamik mock backend ve
test Vite sunucusu portları alır. Chat websocket senaryoları aynı izole frontend/backend çiftini seri biçimde
paylaşır; mock servis tüm arayüzlerde dinlerken Vite proxy bağlantısı IPv4 `127.0.0.1` üzerinden kurulur.
Readiness kontrolü yalnız HTTP cevabını değil SPA HTML kökünü ve dönüştürülmüş giriş modülünü de bekler.
...
>
> Sandbox pytest doğrulamaları `pytest: not found` hatasına düşmemek için çıplak `pytest`,
> `python -m pytest` ve `uv run pytest` komutlarını aynı pre-flight hattına alır; önce
